...
RegisterNetEvent('qb-pawnshop:client:meltItems', function(item)
    ...
    if meltingItem then
        ...
        if meltingItem.amount ~= nil then
			if tonumber(meltingItem.amount) > 20 then
                QBCore.Functions.Notify('Cannot process more than 20 at a time', 'error')
				return
            elseif tonumber(meltingItem.amount) > 0 then
