...
local function XanaxEffect() -- NOT USED move to oxy
    local startStamina = Config.DrugDurations
    local ped = PlayerPedId()
	-- SetRunSprintMultiplierForPlayer(PlayerId(), 1.49)
    while startStamina > 0 do
...
local function AmoxixillinEffect()
    local startStamina = Config.DrugDurations
    local ped = PlayerPedId()
	-- SetRunSprintMultiplierForPlayer(PlayerId(), 1.49)
    while startStamina > 0 do
...
RegisterNetEvent('hospital:client:UseMorphine', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_morphine', 'Taking Morphine', 4000, false, true, {
        ...
        TriggerServerEvent('hospital:server:removeMorphine')
        TriggerEvent('qb-inventory:client:ItemBox', QBCore.Shared.Items['morphine'], 'remove')
...
RegisterNetEvent('hospital:client:UseIbuprofen', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_ibuprofen', 'Taking Ibuprofen', 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(ped, 'mp_suicide', 'pill', 1.0)
        TriggerServerEvent('hospital:server:removeIbuprofen')
        TriggerEvent('qb-inventory:client:ItemBox', QBCore.Shared.Items['ibuprofen'], 'remove')
        TriggerServerEvent('hud:server:RelieveStress', math.random(25, 75))
        RemoveBleed(1)
...
RegisterNetEvent('hospital:client:UseOxycodone', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_oxycodone', 'Taking Oxycodone', 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(ped, 'mp_suicide', 'pill', 1.0)
        TriggerServerEvent('hospital:server:removeOxycodone')
        TriggerEvent('qb-inventory:client:ItemBox', QBCore.Shared.Items['oxycodone'], 'remove')
        TriggerServerEvent('hud:server:RelieveStress', math.random(25, 75))
        OxyEffect()
		if painkillerAmount < 3 then
            painkillerAmount = painkillerAmount + 1
...
RegisterNetEvent('hospital:client:UseXanax', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_xanax', 'Taking Xanax', 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(ped, 'mp_suicide', 'pill', 1.0)
        TriggerServerEvent('hospital:server:removeXanax')
        TriggerEvent('qb-inventory:client:ItemBox', QBCore.Shared.Items['xanax'], 'remove')
...
RegisterNetEvent('hospital:client:UseAmoxixillin', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_amoxixillin', 'Taking Amoxixillin', 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(ped, 'mp_suicide', 'pill', 1.0)
        TriggerServerEvent('hospital:server:removeAmoxixillin')
        TriggerEvent('qb-inventory:client:ItemBox', QBCore.Shared.Items['amoxixillin'], 'remove')
...
RegisterNetEvent('hospital:client:UseIfaks', function()
    ...
    QBCore.Functions.Progressbar('use_bandage', Lang:t('progress.ifaks'), 3000, false, true, {
        ...
        SetEntityHealth(ped, GetEntityHealth(ped) + Config.IfakHeal)
        if painkillerAmount < 3 then
            painkillerAmount = painkillerAmount + 1
        end
        PainKillerLoop()
...
RegisterNetEvent('hospital:client:UseBandage', function()
    ...
    QBCore.Functions.Progressbar('use_bandage', Lang:t('progress.bandage'), 4000, false, true, {
        ...
        SetEntityHealth(ped, GetEntityHealth(ped) + Config.BandageHealAmount)
        if math.random(1, 100) < 50 then
            RemoveBleed(1)
        end
