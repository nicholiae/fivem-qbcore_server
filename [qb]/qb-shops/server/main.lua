...
-- AddEventHandler('onResourceStart', function(resourceName)
    -- if GetCurrentResourceName() ~= resourceName then return end
		-- mychosenlocServer = math.random(#Config.bmLocas)
		-- print("mychosenlocServer on start: "..tostring(mychosenlocServer))
-- end)

QBCore.Functions.CreateCallback('QB-Shops:server:blackmarket', function(_, cb)
	-- print('server CB:'..tostring(cb))
	-- print('server Config.Choice:'..tostring(Config.Choice))
    cb(Config.Choice)
end)
...
local function deliveryPay(source, shop)
    ...
    if math.random(100) <= Config.RewardChance then exports['qb-inventory']:AddItem(source, Config.RewardItem, 1, false, false, 'qb-shops:deliveryPay') end
end
...
RegisterNetEvent('qb-shops:server:openShop', function(data)
    ...
    local items = {}
	local shopSociety = Config.Locations[shopName].society
	-- print(shopSociety)
...
QBCore.Commands.Add('WhereIsBlackMarket', 'Tell me where the blackmarket is', { }, false, function(source, args)
	local src = source
	local message
	local ped = GetPlayerPed(src)
	local Player = QBCore.Functions.GetPlayer(source)
	TriggerClientEvent('QBCore:Notify', src, 'BlackMarket is located: '..tostring(Config.Choice))
	TriggerClientEvent('QB-Shops:Client:setPoint', src)
