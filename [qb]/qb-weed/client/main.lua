...
-- NEW: Calculate expected yield multiplier for display
local function GetYieldMultiplier(health)
    ...
    elseif health >= QBWeed.YieldSystem.minimumHealthThreshold then
        ...
        local yieldRange = 100 - QBWeed.YieldSystem.minimumYieldPercent
        return math.floor(((health - QBWeed.YieldSystem.minimumHealthThreshold) / healthRange) * yieldRange + QBWeed.YieldSystem.minimumYieldPercent)
    else
        return QBWeed.YieldSystem.minimumYieldPercent
    end
end
    ...
    CreateThread(function()
        while currentHouse do
            ...
            if plantSpawned and closestPlant ~= 0 then
                ...
                -- NEW: Calculate yield percentage
                local yieldPercent = GetYieldMultiplier(plantData['plantStats']['health'])
                
                -- NEW: Color coding for health and nutrition
                local healthColor = '~g~'  -- Green
                if plantData['plantStats']['health'] < QBWeed.YieldSystem.perfectHealthThreshold then
                    healthColor = '~o~'  -- Orange
                end
                if plantData['plantStats']['health'] < QBWeed.YieldSystem.minimumHealthThreshold then
                    healthColor = '~r~'  -- Red
                end
                
                local nutritionColor = '~g~'  -- Green
                if plantData['plantStats']['food'] < QBWeed.YieldSystem.perfectHealthThreshold then
                    nutritionColor = '~o~'  -- Orange
                end
                if plantData['plantStats']['food'] < 50 then
                    nutritionColor = '~r~'  -- Red (stops growth)
                end
                
                local plantInfoLabel = Lang:t('text.sort') .. ' ~g~' .. plantData['plantSort']['label'] .. '~w~ [' .. plantData['plantStats']['gender'] .. '] | ' .. Lang:t('text.nutrition') .. ' ' .. nutritionColor .. plantData['plantStats']['food'] .. '% ~w~ | ' .. Lang:t('text.health') .. ' ' .. healthColor .. plantData['plantStats']['health'] .. '%'
                local plantStageLabel = Lang:t('text.stage') .. ' ~b~' .. QBWeed.StageLabels[plantData['plantStats']['stage']] .. ' ~w~' .. Lang:t('text.progress') .. ' ~b~' .. plantData['plantStats']['progress'] .. '%'
                local plantActionLabel

                if plantData['plantStats']['health'] > 0 and plantData['plantStage'] == plantData['plantStats']['highestStage'] then
                    status = 1
                    -- NEW: Show yield percentage at harvest stage
                    plantStageLabel = '~w~Expected Yield: ~b~' .. yieldPercent .. '% ~w~(' .. math.floor(18 * (yieldPercent/100)) .. ' units avg)'
                    plantActionLabel = Lang:t('text.harvest_plant')
                    
                    -- NEW: Add warning if health is low
                    if plantData['plantStats']['health'] < QBWeed.YieldSystem.perfectHealthThreshold then
                        plantActionLabel = plantActionLabel .. ' ~o~(Low Yield - Consider Fertilizing)'
                    end
                elseif plantData['plantStats']['health'] == 0 then
                    ...
                    -- NEW: Add yield estimate for growing plants
                    if QBWeed.YieldSystem.enabled then
                        plantStageLabel = plantStageLabel .. ' ~w~| Expected Yield: ~b~' .. yieldPercent .. '%'
                    end
                end
                ...
                if status > 0 and IsControlJustPressed(0, 38) then
                    ...
                    QBCore.Functions.Progressbar('remove_weed_plant', Lang:t((status == 1) and 'text.harvesting_plant' or 'text.removing_the_plant'), 8000, false, true, {
                        ...
                        ClearPedTasks(ped)
                        if status == 1 then
                            local amount = math.floor(math.random(0, (plantData['plantStats']['gender'] == 'M') and 1 or 3) * (yieldPercent/100))
							if amount < 1 then amount = 0 end
							if amount > 3 then amount = 3 end
...
RegisterNetEvent('qb-weed:client:placePlant', function(type, item)
	...
	if player.name == 'Nicholiae' or player.name == 'ndsha' then
		ClearPedTasks(ped)
...
RegisterNetEvent('qb-weed:client:FoodEmAll', function()
    if not currentHouse then return end
    
    TriggerServerEvent('qb-weed:server:queitup', currentHouse)
end)

RegisterNetEvent('qb-weed:client:harvEmAll', function()
    if not currentHouse then return end
    
    TriggerServerEvent('qb-weed:server:haritup', currentHouse)
end)