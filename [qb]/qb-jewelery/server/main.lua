...
RegisterNetEvent('qb-jewellery:server:vitrineReward', function(vitrineIndex)
    ...
    -- if cachedPoliceAmount[source] >= Config.RequiredCops then -- removes cops requirement instead cops count scales rewards
        if plrPed then
            ...
            if dist <= 25.0 then
				...
				amount = amount * min(cachedPoliceAmount[source] / Config.RequiredCops, 1.0) -- scale by cop count compared to requiredcops
