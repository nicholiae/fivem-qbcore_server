...
local WebHook = 'https://discord.com/api/webhooks/1265153565006102621/rtOxc69j2_z-DelF6nI8zvOOSPdHb0OpvWnJmUBi13VCL9DedDfGP7-xVUmSU1RUaC28'
...
QBCore.Functions.CreateCallback('qb-phone:server:GetCurrentLawyers', function(_, cb)
    ...
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == 'lawyer' or Player.PlayerData.job.name == 'realestate' or Player.PlayerData.job.name == 'reporter' or
                    Player.PlayerData.job.name == 'mechanic' or Player.PlayerData.job.name == 'taxi' or Player.PlayerData.job.name == 'supreme' or Player.PlayerData.job.name == 'mayor' or
                    Player.PlayerData.job.name == 'police' or Player.PlayerData.job.name == 'tuners' or Player.PlayerData.job.name == 'customs' or Player.PlayerData.job.name == 'ambulance' or 
					Player.PlayerData.job.name == 'mechj' or Player.PlayerData.job.name == 'pjsauto' or Player.PlayerData.job.name == 'firefighter') and
...
RegisterNetEvent('qb-phone:server:AddCrashTransaction', function(data)
    local Player = QBCore.Functions.GetPlayerByCitizenId(data.cid)
	if Player == nil then
		Player = QBCore.Functions.GetOfflinePlayerByCitizenId(data.cid)
	end
    MySQL.insert('INSERT INTO crypto_transactions (citizenid, title, message) VALUES (?, ?, ?)', {
        Player.PlayerData.citizenid,
        data.TransactionTitle,
...
-- AutoPilot Events
RegisterNetEvent('qb-phone:server:AutoPilotSpeedUp', function()
    local src = source
    TriggerClientEvent('NS-AutoPilot:client:SpeedUp', src)
end)

RegisterNetEvent('qb-phone:server:AutoPilotSpeedDown', function()
    local src = source
    TriggerClientEvent('NS-AutoPilot:client:SpeedDown', src)
end)

RegisterNetEvent('qb-phone:server:AutoPilotSetGPSMode', function()
    local src = source
    TriggerClientEvent('NS-AutoPilot:client:SetGPSMode', src)
end)

RegisterNetEvent('qb-phone:server:AutoPilotSetWanderMode', function()
    local src = source
    TriggerClientEvent('NS-AutoPilot:client:SetWanderMode', src)
end)

RegisterNetEvent('qb-phone:server:AutoPilotSetGPSDestination', function()
    local src = source
    TriggerClientEvent('NS-AutoPilot:client:SetGPSDestination', src)
end)

RegisterNetEvent('qb-phone:server:AutoPilotStop', function()
    local src = source
    TriggerClientEvent('NS-AutoPilot:client:StopAutoPilot', src)
end)

RegisterNetEvent('qb-phone:server:AutoPilotToggleErrr44', function()
    local src = source
    TriggerClientEvent('NS-AutoPilot:client:ToggleErrr44', src)
end)

-- ============================================
-- PIMP APP CALLBACK - Uses NS-CallGirl Exports
-- Decay and multiplier logic handled by NS-CallGirl
-- ============================================

QBCore.Functions.CreateCallback('qb-phone:server:GetPimpAppData', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    -- Get max multiplier from NS-CallGirl
    local maxMultiplier = exports['NS-CallGirl']:GetMaxMultiplier()
    ...
    -- Get statistics
    local statistics = MySQL.query.await(
        'SELECT * FROM pimp_statistics WHERE citizenid = ?',
        {citizenid}
    )
    
    if not statistics or not statistics[1] then
        statistics = {{
    ...
    -- Build leaderboard using NS-CallGirl exports
    local leaderboard = {}
    local allPlayers = exports['NS-CallGirl']:GetAllPimpPlayers()
    
    if allPlayers then
        local playerMultipliers = {}
        for _, playerData in ipairs(allPlayers) do
            local pid = playerData.citizenid
            local mult, activeCount = exports['NS-CallGirl']:GetPlayerMultiplier(pid)
            playerMultipliers[#playerMultipliers + 1] = {
                citizenid = pid,
        ...
        table.sort(playerMultipliers, function(a, b)
            return a.current_multiplier > b.current_multiplier
        end)
        
        for i = 1, math.min(10, #playerMultipliers) do
            local entry = playerMultipliers[i]
            entry.name = exports['NS-CallGirl']:GetPlayerName(entry.citizenid)
    ...
    if relationships then
        for i, rel in ipairs(relationships) do
            local prostitutes = {
                [42647445] = { nickname = "The Beach Girl" },
                [3413004] = { nickname = "The Hills Princess" },
                [348382215] = { nickname = "The Socialite" },
                [51789996] = { nickname = "The Business Woman" }
            }
            local prostData = prostitutes[rel.prostitute_model]
            if prostData then
                rel.nickname = prostData.nickname
            end
        end
    end
    
    -- Calculate player's multiplier using NS-CallGirl export
    local totalMultiplier, activeCount, baseMultiplier, relationshipBonus = exports['NS-CallGirl']:GetPlayerMultiplier(citizenid)
