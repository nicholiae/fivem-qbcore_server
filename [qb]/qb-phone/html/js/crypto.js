...
$(document).on('click', '#buy-crypto', function(e){
    ...
    var firstPrice = Math.ceil(Coins * CryptoData.Worth);
    var taxPrice = Math.ceil((Coins * CryptoData.Worth)*0.15);
    var Price = Math.ceil(taxPrice + firstPrice);
...
$(".crypto-action-page-buy-crypto-input-coins").keyup(function(){
    var MoneyInput = this.value
    var firstMoneyAmount = Math.ceil(CryptoData.Worth * MoneyInput)
    var taxMoneyAmount = Math.ceil((CryptoData.Worth * MoneyInput)*0.15)
	var	MoneyAmount = Math.ceil(taxMoneyAmount + firstMoneyAmount)
