...
$(document).on('click', '#myhouse-option-sell', function(e){
    ...
    // Request sell price from server
    $.post('https://qb-phone/GetHouseSellPrice', JSON.stringify({
        HouseData: CurrentHouseData
    }), function(data){
        if (data && data.price !== undefined) {
            $("#sell-house-price").html("Sell Price: $" + data.price.toLocaleString());
        } else {
            $("#sell-house-price").html("Unable to get price");
        }
    });
...
$(document).on('click', '#myhouse-option-sell-confirm', function(e){
    ...
    $.post('https://qb-phone/SellHouse', JSON.stringify({
        ...
        if (Success) {
            QB.Phone.Notifications.Add("fas fa-home", "Houses", "House sold successfully!", "#27ae60", 2500);
            ...
            QB.Phone.Notifications.Add("fas fa-home", "Houses", "Failed to sell house", "#e74c3c", 2500);
            shakeElement(".myhouse-option-sell-container");
        }
    });
});
