    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                } else if (PressedApplication == "taxes") {
					// Trigger NS-Taxes module instead of using internal tax system
					$.post('https://qb-phone/OpenTaxApp', JSON.stringify({}));
                } else if (PressedApplication == "cashapp") {
					// Trigger NS-cashApp module instead of using internal tax system
					$.post('https://qb-phone/OpenCashApp', JSON.stringify({}));
                } else if (PressedApplication == "pimpapp") {
					$.post('https://qb-phone/GetPimpAppData', JSON.stringify({}), function(data){
						SetupPimpApp(data);
					});
                ...
                else if (PressedApplication == "taxiservices") {
                    $.post('https://qb-phone/CallTaxiServices', JSON.stringify({}),function(){
                    })
                }
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "RefreshAlerts":
                QB.Phone.Functions.SetupAppWarnings(event.data.AppData);
                break;
            case "UpdateAutoPilotUI":
                if (typeof SetupAutoPilotApp === 'function') {
                    SetupAutoPilotApp(event.data.data);
                }
                break;
        }
    })
