...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            PhoneData.PlayerData = QBCore.Functions.GetPlayerData()
			if PhoneData.PlayerData.citizenid == 'SHP18906' then
				return
			end
...
-- This creates a bridge between QB-Phone and NS-Taxes module
 
-- NUI Callback to open NS-Taxes UI when tax app is clicked
RegisterNUICallback('OpenTaxApp', function(data, cb)
    -- Trigger the NS-Taxes module to open its UI
    TriggerEvent('NS-Taxes:Client:OpenTaxUI')
    cb('ok')
end)
 
RegisterNUICallback('OpenCashApp', function(_, cb)
    -- Trigger the NS-physicalcash module to open its UI
    TriggerEvent('qb-physicalcash:client:openMenu')
    cb('ok')
end)
...
RegisterNUICallback('GetPimpAppData', function(_, cb)
    QBCore.Functions.TriggerCallback('qb-phone:server:GetPimpAppData', function(data)
        cb(data)
    end)
end)
...
RegisterNUICallback('GetHouseSellPrice', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-phone:server:GetHouseSellPrice', function(price)
        cb({price = price})
    end, data.HouseData)
end)

RegisterNUICallback('SellHouse', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-phone:server:SellHouse', function(Success)
        cb(Success)
    end, data.HouseData)
end)
...
RegisterNUICallback('CallTaxiServices', function(_, cb)
	TriggerEvent('NS:NpcTaxi:call')
end)
...
RegisterNUICallback('CallTaxi', function(_, cb)
	TriggerEvent('CxC:NpcTaxi:call', GetEntityCoords(PlayerPedId()))
	cb('ok')
end)

RegisterNUICallback('DismissTaxi', function(_, cb)
	TriggerEvent('CxC:Taxi:cancelTaxi', true)
	cb('ok')
end)

RegisterNUICallback('ResetTaxi', function(_, cb)
	-- Reset functionality would need to be implemented in NS-TaxiServices
	-- For now, we'll just cancel any active taxi
	TriggerEvent('CxC:Taxi:cancelTaxi', true)
	cb('ok')
end)

-- AutoPilot App Callbacks
RegisterNUICallback('AutoPilotCheckVehicle', function(_, cb)
	local ped = PlayerPedId()
	local inVehicle = IsPedInVehicle(ped, GetVehiclePedIsIn(ped, false), false)
	cb(inVehicle)
end)

-- Request current autopilot status when app opens
RegisterNUICallback('AutoPilotAppOpened', function(_, cb)
	TriggerEvent('NS-AutoPilot:client:GetStatus')
	cb('ok')
end)

RegisterNUICallback('AutoPilotToggle', function(_, cb)
	TriggerEvent('AutoPilot:client:toggleAutoPilot')
	cb('ok')
end)

RegisterNUICallback('AutoPilotSpeedUp', function(_, cb)
	TriggerServerEvent('qb-phone:server:AutoPilotSpeedUp')
	cb('ok')
end)

RegisterNUICallback('AutoPilotSpeedDown', function(_, cb)
	TriggerServerEvent('qb-phone:server:AutoPilotSpeedDown')
	cb('ok')
end)

RegisterNUICallback('AutoPilotSetGPSMode', function(_, cb)
	TriggerServerEvent('qb-phone:server:AutoPilotSetGPSMode')
	cb('ok')
end)

RegisterNUICallback('AutoPilotSetWanderMode', function(_, cb)
	TriggerServerEvent('qb-phone:server:AutoPilotSetWanderMode')
	cb('ok')
end)

RegisterNUICallback('AutoPilotSetGPSDestination', function(_, cb)
	TriggerServerEvent('qb-phone:server:AutoPilotSetGPSDestination')
	cb('ok')
end)

RegisterNUICallback('AutoPilotStop', function(_, cb)
	TriggerServerEvent('qb-phone:server:AutoPilotStop')
	cb('ok')
end)

RegisterNUICallback('AutoPilotToggleErrr44', function(_, cb)
	TriggerServerEvent('qb-phone:server:AutoPilotToggleErrr44')
	cb('ok')
end)

-- AutoPilot UI Update Handler
RegisterNetEvent('qb-phone:client:UpdateAutoPilotUI')
AddEventHandler('qb-phone:client:UpdateAutoPilotUI', function(data)
	SendNUIMessage({
		action = "UpdateAutoPilotUI",
		data = data
	})
end)
