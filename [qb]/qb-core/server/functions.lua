...
local function CalculateTotalRelationshipBonusForPayChecks(citizenid)
...
function PaycheckInterval()
    if next(QBCore.Players) then
        for _, Player in pairs(QBCore.Players) do
            if Player then
                ...
                Player.Functions.AddMoney('starbucks', 0.05, 'paycheck')
                TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, 'Thanks for staying active! You have earned Star Bucks!', 'success', 5000)
                local payment = (((QBShared.Jobs[Player.PlayerData.job.name]['grades'][tostring(Player.PlayerData.job.grade.level)].payment) * QBConfig.Money.PayCheckMultiplier) * ((Player.PlayerData.metadata.pimpmultiplier or 1) + (CalculateTotalRelationshipBonusForPayChecks(Player.PlayerData.citizenid) or 0)))
                if not payment then payment = Player.PlayerData.job.payment end
				local tax = math.floor(payment * QBCore.Config.Money.TaxRate)
				payment = payment - tax
                if Player.PlayerData.job and payment > 0 and (QBShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty) then
					if QBCore.Config.Money.PayCheckSociety then
                        local account = exports['qb-banking']:GetAccountBalance(Player.PlayerData.job.name)
						-- print('Player.PlayerData.job:'..tostring(Player.PlayerData.job))
						-- print('account:'.. tostring(account))
						-- print('payment:' .. tostring(payment))
                        if account ~= 0 then          -- Checks if player is employed by a society
                            if account < payment then -- Checks if company has enough money to pay society
								
                                TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('error.company_too_poor'), 'error', 5000)
                            else
                                Player.Functions.AddMoney('bank', payment, 'paycheck')
                                exports['qb-banking']:RemoveMoney(Player.PlayerData.job.name, payment+tax, 'Employee Paycheck')
                                TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('info.received_paycheck', { value = payment, value2 = tax }), 'success', 10000)
                            end
							-- log taxes paid
							local PlayerDatas = MySQL.prepare.await('SELECT * FROM `playerpaidtaxes` where `citizenid` = ?', { Player.PlayerData.citizenid })
							if not PlayerDatas then
								local insertSuccess = MySQL.insert.await('INSERT INTO `playerpaidtaxes` (citizenid,PaidTaxes) VALUES (?,?)', { Player.PlayerData.citizenid, tax })
								TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('info.taxes_paid', { value = tax }), 'success', 10000)
							else
								local PaidAlready = MySQL.prepare.await('SELECT `PaidTaxes` FROM `playerpaidtaxes` where `citizenid` = ?', { Player.PlayerData.citizenid })
								local taxes = PaidAlready + tax
								local updateSuccess = MySQL.update.await('UPDATE playerpaidtaxes SET PaidTaxes = ? WHERE citizenid = ?', { taxes, Player.PlayerData.citizenid })
								TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('info.taxes_paid', { value = taxes }), 'success', 10000)
							end
                        elseif Player.PlayerData.job.name == 'unemployed' then
							Player.Functions.AddMoney('bank', payment, 'paycheck')
							TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('info.received_unemployement', { value = payment }), 'success', 10000)
						else
                            TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('error.company_too_poor'), 'error', 5000)
                        end
                    else
    ...
    SetTimeout(((QBCore.Config.Money.PayCheckTimeOut * (60 * 1000)) * QBCore.Config.Money.PayCheckMultiplier), PaycheckInterval)
end
