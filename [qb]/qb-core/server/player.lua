...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.SetJob(job, grade)
        ...
        if jobGradeInfo then
            ...
            self.PlayerData.job.isowner = jobGradeInfo.isowner or false
        end
    ...
    function self.Functions.SetRep(rep, amount)
        ...
        self.PlayerData.metadata['rep'][rep] = addAmount
        self.Functions.UpdatePlayerData()
    end
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
			local account = exports['qb-banking']:GetAccountBalance(self.PlayerData.job.name)
			if reason == 'paycheck' then
				if amount > 100000 then
					TriggerEvent('qb-log:server:CreateLog', 'playermoneyexcess', 'AddMoney', 'yellow', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason .. ' from account: ' .. self.PlayerData.job.name .. ' with balance of: ' .. account, true)
				else
					TriggerEvent('qb-log:server:CreateLog', 'paychecks', 'AddMoney', 'yellow', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason .. ' from account: ' .. self.PlayerData.job.name .. ' with balance of: ' .. account)
				end
			else
				if amount > 100000 then
					TriggerEvent('qb-log:server:CreateLog', 'playermoneyexcess', 'AddMoney', 'lightgreen', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
				else
