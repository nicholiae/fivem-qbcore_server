...
local function PoliceCall()
	...
	local level = 3
	local message = "You are now wanted for Armed Robbery"
	-- QBCore.Functions.Notify(message, 'error')
	TriggerServerEvent('NS-Wantedhandler:server:addStars', pid, level, message)
    TriggerServerEvent('police:server:policeAlert', 'Armed Robbery in progress')
end
    ...
    while true do
        ...
        for k in pairs(Config.Registers) do
            ...
            if dist <= 1 and Config.Registers[k].robbed then
                inRange = true
                DrawText3Ds(Config.Registers[k][1], Lang:t('text.the_cash_register_is_empty'))
            end
        end
        if not inRange then
            Wait(2000)
    ...
    while true do
        ...
        if QBCore ~= nil then
            ...
            for safe, _ in pairs(Config.Safes) do
                local dist = #(pos - Config.Safes[safe][1])
                if dist < 3 then
                    inRange = true
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe][1], Lang:t('text.try_combination'))
                            ...
                            DrawText3Ds(Config.Safes[safe][1], Lang:t('text.safe_opened'))
                        end
                    end
                end
            end
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
	...
	for k in pairs(Config.Registers) do
		...
		local pos = GetEntityCoords(ped)
		local dist = #(pos - Config.Registers[k][1])
		if dist <= 3 and not Config.Registers[k].robbed then
			if CurrentCops >= Config.MinimumStoreRobberyPolice then
...
RegisterNUICallback('success', function(_, cb)
    if currentRegister ~= 0 then
        ...
        local lockpickTime = Config.EmptyRegisterTime
