...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
	...
	-- print(Config.Registers[register][1])
    if #(playerCoords - Config.Registers[register][1]) > 3.0 or (not Config.Registers[register].robbed and not isDone) or (Config.Registers[register].time <= 0 and not isDone) then
        local reason = 'Store Robbery Exploit Register'
    ...
    if isDone then
        ...
        local info = {
            worth = math.random(cashA, cashB-700)
        }
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    local playerPed = GetPlayerPed(src)
    local playerCoords = GetEntityCoords(playerPed)
    if #(playerCoords - Config.Safes[safe][1]) > 3.0 or Config.Safes[safe].robbed then
        local reason = 'Store Robbery Exploit Safe'
		TriggerEvent('qb-log:server:CreateLog', 'bans', 'Player Kicked', 'red', string.format('%s was kicked by %s for %s', GetPlayerName(p), 'ACS', reason), true)
		return DropPlayer(src, 'Attempted exploit abuse')
    end
    local bags = math.random(10, 30)
    local info = {
    ...
    if luck <= 20 then
        exports['qb-inventory']:AddItem(src, 'electronickit', math.random(3, 25), false, false, 'qb-storerobbery:server:SafeReward')
        TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['rolex'], 'add')
        if luck >= odd then
            Wait(500)
            exports['qb-inventory']:AddItem(src, 'trojan_usb', 1, false, false, 'qb-storerobbery:server:SafeReward')
            TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['trojan_usb'], 'add')
        end
    end
     exports['qb-inventory']:RemoveItem(src, 'stickynote', 1, false, 'qb-storerobbery:server:takeMoney')
