...
function LoadInventory(source, citizenid)
	...
	local logMessage = string.format('**(citizenid: %s | id: %s)** inventory saved: %s', citizenid, source, json.encode(loginventory))
	TriggerEvent('qb-log:server:CreateLog', 'saveinventory', 'Load Inventory', 'green', logMessage)
...
function SaveInventory(source, offline)
	...
	local logMessage = string.format('**%s (citizenid: %s | id: %s)** inventory saved: %s', GetPlayerName(source), PlayerData.citizenid, source, json.encode(items))
	TriggerEvent('qb-log:server:CreateLog', 'saveinventory', 'Save Inventory', 'blue', logMessage)
...
function SetInventory(source, items)
	...
	TriggerEvent('qb-log:server:CreateLog', 'setinventory', 'Set Inventory', 'blue', logMessage)
    -- end
end
...
function ClearInventory(source, filterItems)
    ...
    if not player.Offline then
        ...
        TriggerEvent('qb-log:server:CreateLog', 'clearinventory', 'Clear Inventory', 'red', logMessage)
...
function OpenInventory(source, identifier, data)
	...
	QBCore.Functions.TriggerCallback('NS-BattleRoyal:server:isGameActiveAndPlayerInvolved', function(isActive)
		if isActive then
			if identifier then
				if identifier:find('glovebox', 1, true) then 
					-- print("glovebox open during battleroyal identifier: "..tostring(identifier))
					TriggerClientEvent('QBCore:Notify', source, 'This inventory is blacklisted right now', 'warning')
					CloseInventory(source, identifier)
					-- identifier.isOpen = true
					local logMessage = string.format('**%s (citizenid: %s | id: %s)** InvName:%s PlayerItems: %s', GetPlayerName(source), QBPlayer.PlayerData.citizenid, source, identifier, json.encode(QBPlayer.PlayerData.items))
					TriggerEvent('qb-log:server:CreateLog', 'lootcheats', 'Open Glovebox', 'red', logMessage)
					return
				elseif identifier:find('trunk', 1, true) then 
					-- print("trunk opened during battleroyal identifier: "..tostring(identifier))
					TriggerClientEvent('QBCore:Notify', source, 'This inventory is blacklisted right now', 'warning')
					CloseInventory(source, identifier)
					-- identifier.isOpen = true
					local logMessage = string.format('**%s (citizenid: %s | id: %s)** InvName:%s PlayerItems: %s', GetPlayerName(source), QBPlayer.PlayerData.citizenid, source, identifier, json.encode(QBPlayer.PlayerData.items))
					TriggerEvent('qb-log:server:CreateLog', 'lootcheats', 'Open Trunk', 'red', logMessage)
					return
				elseif identifier:find('drop', 1, true) then 
					-- print("drop bag during battleroyal identifier: "..tostring(identifier))
					TriggerClientEvent('QBCore:Notify', source, 'This inventory is blacklisted right now', 'warning')
					CloseInventory(source, identifier)
					-- identifier.isOpen = true
					return
				...
				-- else
				-- TriggerClientEvent('QBCore:Notify', source, 'Opened '..tostring(identifier), 'success')
				-- end
			end
		-- else
			-- print("not active")
	...
	local logMessage = string.format('**%s (citizenid: %s | id: %s)** InvName:%s items in: %s', GetPlayerName(source), QBPlayer.PlayerData.citizenid, source, identifier, json.encode(inventory.items))
    TriggerEvent('qb-log:server:CreateLog', 'openinventory', 'Open Inventory', 'yellow', logMessage)
end
...
--- @param identifier string|nil The identifier of the inventory to open.
--- @param data table|nil Additional data for initializing the inventory.
function createLootInventory(source, identifier, data)
    
    local inventory = Inventories[identifier]
    -- TriggerClientEvent('QBCore:Notify', source, tostring(identifier)..' Inventory Created', 'primary', 5000)
    -- TriggerClientEvent('QBCore:Notify', source, tostring(inventory)..' Inventory Created', 'primary', 5000)
	...
	local logMessage = string.format('**InvName**:%s ', identifier)
    TriggerEvent('qb-log:server:CreateLog', 'lootCrates', 'Crates Created', 'yellow', logMessage)
end

exports('createLootInventory', createLootInventory)
...
function AddItem(identifier, item, amount, slot, info, reason)
    ...
    if totalWeight + (itemInfo.weight * amount) > inventoryWeight then
        ...
        TriggerClientEvent('QBCore:Notify', identifier, 'Not enough weight available, empty your inventory', 'error')
        return false
    end
    ...
    if not updated then
        ...
        if not slot then
			...
			TriggerClientEvent('QBCore:Notify', identifier, 'Not enough slots available, empty your inventory', 'error')
            return false
        end
...
function RemoveItem(identifier, item, amount, slot, reason)
    ...
    if player then 
		...
		-- Trigger immediate client update if item was completely removed
		if inventoryItem.amount <= 0 then
			TriggerClientEvent('qb-inventory:client:updateInventory', player.PlayerData.source)
		end
	end
    local invName = player and GetPlayerName(identifier) .. ' (' .. identifier .. ')' or identifier
