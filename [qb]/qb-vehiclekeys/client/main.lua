        ...
        while true do
            ...
            if LocalPlayer.state.isLoggedIn then
                        ...
                        if IsEntityDead(driver) then
                            if not isTakingKeys then
                                ...
                                QBCore.Functions.Progressbar('steal_keys', Lang:t('progress.takekeys'), 2500, false, false, {
									...
									local s1, s2 = GetStreetNameAtCoord(GetEntityCoords(playerPed).x, GetEntityCoords(playerPed).y, GetEntityCoords(playerPed).z)
									...
									if math.random(1, 100) <= 85 and not QBCore.Functions.IsWearingGloves() then
										TriggerServerEvent('evidence:server:CreateFingerDropInCar', plate,streetLabel)
                ...
                if IsPedInAnyVehicle(ped, false) and not IsHotwiring then
					...
					if jobName == 'police' and not HasKeys(plate) and onDuty then
						TriggerServerEvent('qb-vehiclekeys:server:AcquireVehicleKeys', plate)
					end
					if jobName == 'ambulance' and not HasKeys(plate) and onDuty then
						TriggerServerEvent('qb-vehiclekeys:server:AcquireVehicleKeys', plate)
					end
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
			...
			TriggerServerEvent('sayer-gangs:AddRepClient', 'carjacking')
        else
