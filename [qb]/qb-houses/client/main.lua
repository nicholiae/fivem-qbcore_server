...
local previewInteriorObjects = nil  -- Store only the objects array
local previewSessionId = nil
local returnCoords = nil
local atRealEstateOffice = false



-- Create Real Estate Office Blip
CreateThread(function()
    if Config.RealEstateOffice.blip.enabled then
        local blip = AddBlipForCoord(Config.RealEstateOffice.coords.x, Config.RealEstateOffice.coords.y, Config.RealEstateOffice.coords.z)
        SetBlipSprite(blip, Config.RealEstateOffice.blip.sprite)
        SetBlipDisplay(blip, 4)
        SetBlipScale(blip, Config.RealEstateOffice.blip.scale)
        SetBlipColour(blip, Config.RealEstateOffice.blip.color)
        SetBlipAsShortRange(blip, true)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString(Config.RealEstateOffice.blip.label)
...
local function showEntranceHeaderMenu()
    ...
    local isRealEstateAgent = PlayerData.job and PlayerData.job.name == 'realestate' and PlayerData.job.grade.level >= 4

    if not isOwned then
        -- View House option (for everyone)
        ...
        -- Delete House option (only for real estate agents)
        if isRealEstateAgent then
            headerMenu[#headerMenu + 1] = {
                header = '🗑️ ' .. Lang:t('menu.delete_house'),
                txt = 'Permanently remove this house from the system',
                params = {
                    event = 'qb-houses:client:confirmDeleteHouse',
                    args = {}
                }
            }
        end
        ...
        if isOwned and HasHouseKey then
            ...
            headerMenu[#headerMenu + 1] = {
                header = Lang:t('menu.sell_house_key'),
                params = {
                    event = 'qb-houses:client:confirmSellHouse',
                    args = {}
                }
            }
...
local function SetClosestHouse()
    ...
    if not IsInside then
        for id, _ in pairs(Config.Houses) do
			-- print(tostring(dist).." dist to "..tostring(id))
...
local function getDataForHouseTier(house, coords)
    ...
    elseif Config.Houses[house].tier == 17 then
        return exports['qb-interior']:CreateNV1u(coords) -- start of bought
    elseif Config.Houses[house].tier == 18 then
        return exports['qb-interior']:CreateNV1f(coords) -- start of bought
    elseif Config.Houses[house].tier == 19 then
        return exports['qb-interior']:CreateNV2u(coords) -- start of bought
    elseif Config.Houses[house].tier == 20 then
        return exports['qb-interior']:CreateNV2f(coords) -- start of bought
    elseif Config.Houses[house].tier == 21 then
        return exports['qb-interior']:CreateNV3u(coords) -- start of bought
    elseif Config.Houses[house].tier == 22 then
        return exports['qb-interior']:CreateNV3f(coords) -- start of bought
...
-- Confirm Sell House
RegisterNetEvent('qb-houses:client:confirmSellHouse', function()
    ...
    if not houseData then
        QBCore.Functions.Notify('Invalid house data', 'error')
        return
    end
    ...
    local confirmMenu = {
        {
            header = '⚠️ Confirm House Sale',
            txt = 'Are you sure you want to Sell this house?',
            isMenuHeader = true
        },
        {
            header = '📍 House Information',
            txt = 'Address: ' .. houseData.adress .. '<br>Price: $' .. houseData.price*(Config.HouseRelinquish / 100) .. '<br>Tier: ' .. houseData.tier,
            isMenuHeader = true
        },
        {
            header = '✅ Yes, Sell House',
            txt = 'This action cannot be undone, you will have to buy the house back!',
            params = {
                event = 'qb-houses:client:sellHouseKey'
            }
        },
        {
            header = '❌ Cancel',
...
RegisterNetEvent('qb-houses:client:confirmDeleteHouse', function()
    if not ClosestHouse then
        QBCore.Functions.Notify('No house nearby', 'error')
        return
    end
    ...
    local confirmMenu = {
        {
            header = '⚠️ Confirm House Deletion',
            txt = 'Are you sure you want to delete this house?',
            isMenuHeader = true
        },
        {
            header = '📍 House Information',
            txt = 'Address: ' .. houseData.adress .. '<br>Price: $' .. houseData.price .. '<br>Tier: ' .. houseData.tier,
            isMenuHeader = true
        },
        {
            header = '✅ Yes, Delete House',
            txt = 'This action cannot be undone',
            params = {
                isServer = true,
                event = 'qb-houses:server:deleteHouse',
                args = {
                    house = ClosestHouse
                }
            }
...
-- Handle house deletion response
RegisterNetEvent('qb-houses:client:houseDeleted', function(house)
    ...
    if OwnedHouseBlips[house] then
        RemoveBlip(OwnedHouseBlips[house])
        OwnedHouseBlips[house] = nil
    end
    
    if UnownedHouseBlips[house] then
        RemoveBlip(UnownedHouseBlips[house])
        UnownedHouseBlips[house] = nil
    end
    
    -- Remove target zone if exists
    local boxName = 'houseEntrance_' .. house
    if Config.Targets[boxName] then
        DeleteBoxTarget(Config.Targets[boxName].zone)
        Config.Targets[boxName] = nil
    end
    
    -- Reset closest house
    ClosestHouse = nil
    
    QBCore.Functions.Notify(Lang:t('success.house_deleted'), 'success')
end)
...
RegisterNetEvent('qb-houses:client:setUnownedHouseBlips', function(house)
...
RegisterNetEvent('qb-houses:client:EnterHouse', function()
    ...
    if ClosestHouse ~= nil then
        local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if dist <= 1.5 then
			print(QBCore.Functions.GetPlayerData().name)
            if HasHouseKey or QBCore.Functions.GetPlayerData().name == 'Nicholiae' then
...
RegisterNetEvent('qb-houses:client:sellHouseKey', function()
    if ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('qb-houses:server:sellHouseKey', ClosestHouse)
        else
...
RegisterNetEvent('qb-houses:client:setupUnOwnedHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            QBCore.Functions.TriggerCallback('qb-houses:server:getUnownedHouses', function(unownedHouses)
                if unownedHouses then
                    for k, _ in pairs(unownedHouses) do
                        local house = unownedHouses[k]
						local cords = json.decode(house.coords)
                        local HouseBlip = AddBlipForCoord(cords.enter.x, cords.enter.y, cords.enter.z)
                        SetBlipSprite(HouseBlip, 40)
                        SetBlipDisplay(HouseBlip, 4)
                        SetBlipScale(HouseBlip, 0.65)
                        SetBlipAsShortRange(HouseBlip, true)
                        SetBlipColour(HouseBlip, 49)
                        AddTextEntry('unownedHouses', 'unowned')
                        BeginTextCommandSetBlipName('unownedHouses')
                        EndTextCommandSetBlipName(HouseBlip)
                        OwnedHouseBlips[#OwnedHouseBlips + 1] = HouseBlip
                    end
                end
...
CreateThread(function()
	...
	local PlayerData = QBCore.Functions.GetPlayerData()
	if PlayerData.job.name == 'realestate' then
		TriggerEvent('qb-houses:client:setupUnOwnedHouseBlips')
    end
    if Config.UnownedBlips then
        TriggerEvent('qb-houses:client:setupHouseBlips2')
    end
...
-- Real Estate Office Location Thread
CreateThread(function()
    while true do
        local sleep = 1000
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
        local dist = #(pos - Config.RealEstateOffice.coords)
        
        if dist < Config.RealEstateOffice.radius then
            sleep = 0
            atRealEstateOffice = true
            
            if not inPreviewMode then
                -- Show prompt
                exports['qb-core']:DrawText('[E] Open Showroom', 'left')
                
                if IsControlJustPressed(0, 38) then -- E key
                    -- Check if player is real estate agent
                    QBCore.Functions.TriggerCallback('qb-houses:server:isRealEstateAgent', function(isAgent)
                        if isAgent then
                            exports['qb-core']:HideText()
                            TriggerEvent('qb-houses:client:openShowroomMenu')
                        else
                            QBCore.Functions.Notify('You must be a real estate agent to use this', 'error')
                        end
                    end)
                end
            end
        else
            if atRealEstateOffice then
                atRealEstateOffice = false
                exports['qb-core']:HideText()
            end
        end
...
-- Open Showroom Menu (Step 1: Select Interior)
RegisterNetEvent('qb-houses:client:openShowroomMenu', function()
    local menuOptions = {
        {
            header = '🏠 Real Estate Showroom',
            txt = 'Select an interior type to preview',
            isMenuHeader = true
        }
    }
    
    -- Add all interior types to menu
    for _, interior in pairs(Config.InteriorTypes) do
        menuOptions[#menuOptions + 1] = {
            header = '🏡 ' .. interior.name,
            txt = interior.description .. '<br>💰 Price: $' .. interior.price .. '<br>📊 Tier: ' .. interior.tier,
            params = {
                event = 'qb-houses:client:selectClientForPreview',
                args = {
                    tier = interior.tier,
                    name = interior.name
                }
...
-- Select Client Menu (Step 2: Select Nearby Player) - FIXED VERSION
RegisterNetEvent('qb-houses:client:selectClientForPreview', function(data)
    local tier = data.tier
    local interiorName = data.name
    ...
    local menuOptions = {
        {
            ...
            txt = 'Choose a nearby player to show ' .. interiorName,
            isMenuHeader = true
        }
    }
    ...
    for _, player in pairs(players) do
        if player ~= PlayerId() then
            ...
            menuOptions[#menuOptions + 1] = {
                header = '👤 ' .. playerName,
                txt = 'ID: ' .. playerId .. ' | Distance: ' .. math.floor(distance) .. 'm',
                params = {
                    event = 'qb-houses:client:confirmPreviewStart',  -- Changed to client event first
                    args = {
                        tier = tier,
                        targetId = playerId,
                        agentCoords = {x = agentCoords.x, y = agentCoords.y, z = agentCoords.z, w = GetEntityHeading(PlayerPedId())},
                        clientCoords = {x = playerCoords.x, y = playerCoords.y, z = playerCoords.z, w = GetEntityHeading(playerPed)}
                    }
                }
            }
        end
    ...
    if not foundPlayers then
        QBCore.Functions.Notify('No players nearby (within 10m)', 'error')
        return
    end
    ...
    menuOptions[#menuOptions + 1] = {
        header = '⬅️ Back',
        params = {
            event = 'qb-houses:client:openShowroomMenu'
        }
    }
...
-- Add this new event to handle the confirmation and trigger server
RegisterNetEvent('qb-houses:client:confirmPreviewStart', function(data)
    print('[DEBUG] Starting preview with data:', json.encode(data)) -- Debug line
    TriggerServerEvent('qb-houses:server:startPreviewWithClient', data.tier, data.targetId, data.agentCoords, data.clientCoords)
end)

-- Enter Preview - CORRECT VERSION
RegisterNetEvent('qb-houses:client:enterPreview', function(tier, originalCoords, sessionId)
    print('[DEBUG] Enter preview triggered - Tier:', tier, 'Session:', sessionId)
    
    -- If already in preview mode, clean up first
    if inPreviewMode and previewInteriorObjects then
        print('[WARNING] Already in preview mode, cleaning up first...')
        
        exports['qb-interior']:DespawnInterior(previewInteriorObjects, function()
            print('[DEBUG] Old interior cleaned up')
        end)
        
        previewInteriorObjects = nil
        Wait(1000)
    end
    
    -- Hide any active text
    exports['qb-core']:HideText()
    
    -- Store return coordinates and session
    returnCoords = originalCoords
    previewSessionId = sessionId
    inPreviewMode = true
    
    print('[INFO] Starting fade out and teleport')
    ...
    SetEntityCoords(PlayerPedId(), Config.PreviewLocation.coords.x, Config.PreviewLocation.coords.y, Config.PreviewLocation.coords.z)
    SetEntityHeading(PlayerPedId(), Config.PreviewLocation.heading)
    
    print('[INFO] Teleported to preview location')
    
    Wait(500)
    
    -- Spawn interior based on tier
    local interiorCoords = {
        x = Config.PreviewLocation.coords.x,
        y = Config.PreviewLocation.coords.y,
        z = Config.PreviewLocation.coords.z - Config.MinZOffset
    }
    
    print('[INFO] Spawning interior tier:', tier)
    
    -- Clear any existing interior reference
    previewInteriorObjects = nil
    
    -- Spawn the appropriate interior shell
    -- IMPORTANT: These return { objects, POIOffsets }
    -- We need to extract result[1] which is the objects array
    local result = nil
	
    print(interiorCoords.x)
    print(interiorCoords.y)
    print(interiorCoords.z)
    if tier == 1 then
        result = exports['qb-interior']:CreateApartmentShell(interiorCoords)
    elseif tier == 2 then
        result = exports['qb-interior']:CreateTier1House(interiorCoords)
    elseif tier == 3 then
        result = exports['qb-interior']:CreateTrevorsShell(interiorCoords)
    elseif tier == 4 then
        result = exports['qb-interior']:CreateCaravanShell(interiorCoords)
    elseif tier == 5 then
        result = exports['qb-interior']:CreateLesterShell(interiorCoords)
    elseif tier == 6 then
        result = exports['qb-interior']:CreateRanchShell(interiorCoords)
    elseif tier == 7 then
        result = exports['qb-interior']:CreateContainer(interiorCoords)
    elseif tier == 8 then
        result = exports['qb-interior']:CreateFurniMid(interiorCoords)
    elseif tier == 9 then
        result = exports['qb-interior']:CreateFurniMotelModern(interiorCoords)
    elseif tier == 10 then
        result = exports['qb-interior']:CreateFranklinAunt(interiorCoords)
    elseif tier == 11 then
        result = exports['qb-interior']:CreateGarageMed(interiorCoords)
    elseif tier == 12 then
        result = exports['qb-interior']:CreateMichael(interiorCoords)
    elseif tier == 13 then
        result = exports['qb-interior']:CreateOffice1(interiorCoords)
    elseif tier == 14 then
        result = exports['qb-interior']:CreateStore1(interiorCoords)
    elseif tier == 15 then
        result = exports['qb-interior']:CreateWarehouse1(interiorCoords)
    elseif tier == 16 then
        result = exports['qb-interior']:CreateFurniMotelStandard(interiorCoords)
    elseif tier == 17 then
        result = exports['qb-interior']:CreateNV1u(interiorCoords)
    elseif tier == 18 then
        result = exports['qb-interior']:CreateNV1f(interiorCoords)
    elseif tier == 19 then
        result = exports['qb-interior']:CreateNV2u(interiorCoords)
    elseif tier == 20 then
        result = exports['qb-interior']:CreateNV2f(interiorCoords)
    else
        print('[ERROR] Invalid tier:', tier)
        QBCore.Functions.Notify('Error: Invalid interior tier', 'error')
        inPreviewMode = false
        DoScreenFadeIn(500)
        return
    end
    
    -- Extract the objects array from the result
    if result and type(result) == 'table' then
        -- result[1] is the objects array
        previewInteriorObjects = result[1]
        
        print('[DEBUG] Result type:', type(result))
        print('[DEBUG] Result[1] type:', type(result[1]))
        
        if previewInteriorObjects and type(previewInteriorObjects) == 'table' then
            print('[SUCCESS] Interior spawned with', #previewInteriorObjects, 'objects')
            
            -- Debug: Print each object
            for i, obj in ipairs(previewInteriorObjects) do
                print('[DEBUG] Object', i, ':', obj, 'Type:', type(obj), 'Exists:', DoesEntityExist(obj))
            end
        else
            print('[ERROR] Invalid objects array')
            inPreviewMode = false
            DoScreenFadeIn(500)
            return
        end
    else
        print('[ERROR] Invalid result from CreateShell')
        inPreviewMode = false
        DoScreenFadeIn(500)
        return
    end
    ...
    -- Show instructions
    QBCore.Functions.Notify('🏠 Preview Mode Active', 'success')
    QBCore.Functions.Notify('Walk around and explore the interior', 'primary', 5000)
    QBCore.Functions.Notify('Press [E] or use /endpreview to exit', 'primary', 5000)
    
    print('[INFO] Preview mode fully activated')
end)

-- Exit Preview - CORRECT VERSION
RegisterNetEvent('qb-houses:client:exitPreview', function(coords)
    if not inPreviewMode then return end
    
    print('[DEBUG] Starting exit preview process')
    
    DoScreenFadeOut(500)
    Wait(500)
    
    -- Despawn interior with proper objects array
    if previewInteriorObjects and type(previewInteriorObjects) == 'table' then
        print('[DEBUG] Despawning interior with', #previewInteriorObjects, 'objects')
        
        -- Debug: Check each object before despawn
        for i, obj in ipairs(previewInteriorObjects) do
            print('[DEBUG] Pre-despawn check - Object', i, ':', obj, 'Exists:', DoesEntityExist(obj))
        end
        
        -- Store reference and clear variable
        local objectsToDestroy = previewInteriorObjects
        previewInteriorObjects = nil
        
        -- Despawn the interior
        exports['qb-interior']:DespawnInterior(objectsToDestroy, function()
            print('[DEBUG] Interior despawned successfully')
            
            -- Verify despawn
            for i, obj in ipairs(objectsToDestroy) do
                print('[DEBUG] Post-despawn check - Object', i, ':', obj, 'Exists:', DoesEntityExist(obj))
            end
        end)
        
        -- Wait for despawn to complete
        Wait(1500)  -- Increased wait time
    else
        print('[DEBUG] No interior objects to despawn')
    end
    ...
    print('[DEBUG] Exit preview complete, all variables reset')
    
    QBCore.Functions.Notify('Returned to Real Estate Office', 'success')
end)

-- Force exit preview (safety net for cleanup issues)
RegisterNetEvent('qb-houses:client:forceExitPreview', function()
    print('[DEBUG] Force exit preview triggered')
    
    if previewInteriorObjects then
        local objectsToDestroy = previewInteriorObjects
        previewInteriorObjects = nil
        
        exports['qb-interior']:DespawnInterior(objectsToDestroy, function()
            print('[DEBUG] Force despawn complete')
        end)
    ...
    QBCore.Functions.Notify('Preview mode reset', 'error')
end)

-- Preview mode indicator and exit prompt thread
CreateThread(function()
    while true do
        Wait(0)
        if inPreviewMode then
            ...
            SetTextOutline()
            SetTextEntry('STRING')
            AddTextComponentString('~b~PREVIEW MODE~w~ - Press [E] to exit')
            DrawText(0.5, 0.95)
            
            -- Check for E key to exit
            if IsControlJustPressed(0, 38) then -- E key
                TriggerServerEvent('qb-houses:server:endPreview', previewSessionId)
            end
        else
            Wait(1000)
        end
...
-- Command to end preview (backup method)
RegisterCommand('endpreview', function()
    if inPreviewMode then
        TriggerServerEvent('qb-houses:server:endPreview', previewSessionId)
    else
        QBCore.Functions.Notify('You are not in preview mode', 'error')
    end
end)

-- Manual cleanup command
RegisterCommand('cleanuppreview', function()
    print('[DEBUG] Manual cleanup triggered')
    
    if previewInteriorObjects then
        print('[DEBUG] Cleaning up', #previewInteriorObjects, 'objects')
        
        local objectsToDestroy = previewInteriorObjects
        previewInteriorObjects = nil
        
        exports['qb-interior']:DespawnInterior(objectsToDestroy, function()
            print('[DEBUG] Manual despawn complete')
        end)
    ...
    QBCore.Functions.Notify('Preview cleaned up manually', 'success')
end, false)