...
local function alertCops()
	...
	local level = 3
	local message = "You are now wanted for BNE"
...
local function spawnPed(house)
	-- print(house)
	local tier = Config.Houses[house]['tier']
	local thisSpawn = Config.PedSpawn[tier][math.random(#Config.PedSpawn[tier])]
	
	local pedLoc = { x = Config.Houses[house].coords.x + thisSpawn.x, y = Config.Houses[house].coords.y + thisSpawn.y, z = Config.Houses[house].coords.z - Config.MinZOffset + thisSpawn.z}
	-- print(pedLoc.x)
	-- print(pedLoc.y)
	-- print(pedLoc.z)
	while not DoesEntityExist(homeOwner) do
		if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then --male
		
			RequestModel("a_m_y_beach_03")
			while not HasModelLoaded("a_m_y_beach_03") do
				Wait(10)
			end
			homeOwner = CreatePed(26, "a_m_y_beach_03", pedLoc.x, pedLoc.y, pedLoc.z, 8.9422, true, true)
		else
			RequestModel("a_f_y_bevhills_04")
			while not HasModelLoaded("a_f_y_bevhills_04") do
				Wait(10)
			end
			homeOwner = CreatePed(26, "a_f_y_bevhills_04", pedLoc.x, pedLoc.y, pedLoc.z, 8.9422, true, true)
		end
		Wait(1000)
		QBCore.Functions.Notify('Someone is home', 'error')
	end
	Wait(1000)
	if DoesEntityExist(homeOwner) then
		local chance = math.random(0,100)
		if chance < Config.WeaponChance then
			GiveWeaponToPed(homeOwner, `weapon_revolver`, 400, false, true)
			-- GiveWeaponToPed(homeOwner, GetHashKey('weapon_revolver'), 400, false, true)
		end
		SetPedCombatAbility(homeOwner, 0)
		SetPedCombatMovement(homeOwner, 3)
		FreezeEntityPosition(homeOwner, false)
		SetPedCombatAttributes(homeOwner, 5, true)
		SetPedAccuracy(homeOwner, 33)
		TaskCombatPed(homeOwner, PlayerPedId(), 0, 16)
		SetPedKeepTask(homeOwner, true)
		QBCore.Functions.Notify('They going to kill you', 'error')
		local thisChance = math.random(1,100)
		if thisChance <= Config.ChanceToAlertPolice then
			alertCops()
		end
	end
...
local function enterRobberyHouse(house)
	...
	if tier == 2 then
		local data = exports['qb-interior']:CreateApartmentRobbery(coords)
		if not data then return end
		houseObj = data[1]
		POIOffsets = data[2]
		inside = true
...
local function leaveRobberyHouse(house)
	...
	if chance < Config.ChanceToAlertPoliceUponLeaving then
		alertCops()
	end
	Wait(250)
	DoScreenFadeOut(250)
...
local function searchCabin(cabin)
	...
	elseif math.random(1, 100) <= 2 and QBCore.Functions.IsWearingGloves() then
		local pos = GetEntityCoords(PlayerPedId())
		TriggerServerEvent('evidence:server:CreateFingerDrop', pos)
            ...
            if not Config.Houses[closestHouse]['opened'] then
                ...
                if usingAdvanced then
                    ...
                    if success then
						...
						local chance = math.random(1, 100)
                        if chance <= Config.ChanceToBreakAdvancedLockPick then
                    ...
                    if success then
						...
						local chance = math.random(1, 100)
                        if chance <= Config.ChanceToBreakLockPick then
