...
RegisterNUICallback('dynamicHygiene', function(_, cb)
    Wait(50)
    Menu.isDynamicHygieneChecked = not Menu.isDynamicHygieneChecked
    TriggerEvent('hud:client:playHudChecklistSound')
    saveSettings()
    cb('ok')
end)

RegisterNUICallback('dynamicBioOne', function(_, cb)
    Wait(50)
    Menu.isDynamicBioOneChecked = not Menu.isDynamicBioOneChecked
    TriggerEvent('hud:client:playHudChecklistSound')
    saveSettings()
    cb('ok')
end)

RegisterNUICallback('dynamicBioTwo', function(_, cb)
    Wait(50)
    Menu.isDynamicBioTwoChecked = not Menu.isDynamicBioTwoChecked
    TriggerEvent('hud:client:playHudChecklistSound')
    saveSettings()
    cb('ok')
end)
...
RegisterNetEvent('hud:client:LoadMap', function()
    ...
    local minimapOffset = -0.008
    local minimapOffsetY = 0.06
    ...
    if Menu.isToggleMapShapeChecked == 'square' then
		...
		if PlayerData.citizenid == 'GUX51858' then
			minimapOffset = 0.005
			minimapOffsetY = 0.04
		end
		if PlayerData.citizenid == 'DOU38972' then
			minimapOffset = 0.005
			minimapOffsetY = 0.04
		end
        ...
        SetMinimapComponentPosition('minimap', 'L', 'B', 0.0 + minimapOffset, -0.047 + minimapOffsetY, 0.1638, 0.183)

        -- icons within map
        SetMinimapComponentPosition('minimap_mask', 'L', 'B', 0.0 + minimapOffset, 0.0 + minimapOffsetY, 0.128, 0.20)

        -- -0.01 = map pulled left
        -- 0.025 = map raised up
        -- 0.262 = map stretched
        -- 0.315 = map shorten
        SetMinimapComponentPosition('minimap_blur', 'L', 'B', -0.01 + minimapOffset, 0.025 + minimapOffsetY, 0.262, 0.300)
    ...
    elseif Menu.isToggleMapShapeChecked == 'circle' then
        ...
        SetMinimapComponentPosition('minimap', 'L', 'B', 0.0050 + minimapOffset, -0.050 + minimapOffsetY, 0.180, 0.258)

        -- icons within map
        SetMinimapComponentPosition('minimap_mask', 'L', 'B', 0.215 + minimapOffset, -0.02 + minimapOffsetY, 0.065, 0.20)

        -- -0.00 = map pulled left
        -- 0.015 = map raised up
        -- 0.252 = map stretched
        -- 0.338 = map shorten
        -- SetMinimapComponentPosition('minimap_blur', 'L', 'B', -0.00 + minimapOffset, 0.015 + minimapOffsetY, 0.252, 0.338)
        SetMinimapComponentPosition('minimap_blur', 'L', 'B', 0.015 + minimapOffset, -0.005 + minimapOffsetY, 0.252, 0.338)
        SetBlipAlpha(GetNorthRadarBlip(), 0)
        SetMinimapClipType(1)
        SetBigmapActive(true, false)
        Wait(50)
...
RegisterNetEvent('hud:client:UpdateNeedsBiology', function(newHygiene, newBioOne, newBioTwo) -- Triggered in qb-core
	hygiene = newHygiene
    bioone = newBioOne
    biotwo = newBioTwo
end)
...
RegisterNetEvent('hud:client:UpdateHygiene', function(newHygiene) -- Add this event with adding hygiene elsewhere
    hygiene = newHygiene
end)

RegisterNetEvent('hud:client:UpdateBioOne', function(newBioOne) -- Add this event with adding bioone elsewhere
    bioone = newBioOne
end)

RegisterNetEvent('hud:client:UpdateBioTwo', function(newBioTwo) -- Add this event with adding stress elsewhere
    biotwo = newBioTwo
end)
...
local function updatePlayerHud(data)
    ...
    if shouldUpdate then
        SendNUIMessage({
            ...
            dynamicHygiene = data[8],
            dynamicBioOne = data[9],
            dynamicBioTwo = data[10],
            dynamicEngine = data[11],
            dynamicNitro = data[12],
            health = data[13],
            playerDead = data[14],
            armor = data[15],
            thirst = data[16],
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports['myFuel']:GetFuel(vehicle))
    end
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if not (IsPedInAnyVehicle(player) and not IsThisModelABicycle(vehicle)) then
                updatePlayerHud({
                    ...
                    Menu.isDynamicHygieneChecked,
                    Menu.isDynamicBioOneChecked,
                    Menu.isDynamicBioTwoChecked,
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['myFuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
...
RegisterNetEvent('hud:client:ShowAccounts', function(type, amount)
	...
	elseif type == 'starbucks' then
		print("nailed it")
        SendNUIMessage({
            action = 'show',
            type = 'starbucks',
            starbucks = Round(amount)
        })
    else
        SendNUIMessage({
            action = 'show',
...
RegisterNetEvent('hud:client:OnMoneyChange', function(type, amount, isMinus)
    ...
    starbucksAmount = PlayerData.money['starbucks']
    SendNUIMessage({
        action = 'updatemoney',
        cash = Round(cashAmount),
        bank = Round(bankAmount),
        starbucks = Round(starbucksAmount),
        amount = Round(amount),
        minus = isMinus,
        type = type
    })
