...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    if bankId == "paleto" then
        ...
        if hasItem then
            ...
            IsDrilling = true
            QBCore.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(6000, 10000), false, true, {
                    ...
                    if currentLocker ~= 0 and not IsDrilling and Config.SmallBanks[i]["isOpened"] and not Config.SmallBanks[i]["lockers"][currentLocker]["isOpened"] and not Config.SmallBanks[i]["lockers"][currentLocker]["isBusy"] then
                        ...
                        if IsControlJustPressed(0, 38) then
							QBCore.Functions.TriggerCallback('NS-StreetTricks:server:canWeCrime', function(results)
								-- print('cornerselling results:'..tostring(results))
								if results ~= 'cityservice' then
