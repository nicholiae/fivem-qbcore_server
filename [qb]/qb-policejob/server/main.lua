...
local function UpdateBlips()
	...
	for k,v in pairs(players) do
        if v and (v.PlayerData.job.name == 'police' or v.PlayerData.job.name == 'ambulance' or v.PlayerData.job.name == 'firefighter') and v.PlayerData.job.onduty then
			-- print(v.PlayerData.name)
			-- for k,v in pairs(value) do
				-- if k == 'PlayerData' then
					-- if v.job.name == 'firefighter' then
    ...
    -- for i = 1, #players do
        ...
        -- if v and (v.PlayerData.job.name == 'police' or v.PlayerData.job.name == 'ambulance' or v.PlayerData.job.name == 'firefighter') and v.PlayerData.job.onduty then
            ...
            -- dutyPlayers[#dutyPlayers + 1] = {
                -- source = v.PlayerData.source,
                -- label = v.PlayerData.metadata['callsign'] or 'firefighter',
                -- job = v.PlayerData.job.name,
                -- location = {
                    -- x = coords.x,
                    -- y = coords.y,
...
QBCore.Commands.Add('writeWarrant', 'Write a warrant', { { name = 'CID', help = 'Target Player CitizenID' }, { name = 'warrantType', help = 'Type of Warrant' }, { name = 'Reasons', help = 'Tell me why' } }, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == 'mayor' and Player.PlayerData.job.grade.level >= Config.LicenseRank then
		local SearchedPlayer = QBCore.Functions.GetPlayerByCitizenId(args[1])
		local reasons = tostring(args[3])
		-- print(SearchedPlayer)
		if SearchPlayer == nil then
			SearchedPlayer = QBCore.Functions.GetOfflinePlayerByCitizenId(args[1])
		end
		-- print(SearchedPlayer)
		local warrantType = tostring(args[2])
		...
		if args[1] then 
			...
			info.type = tostring(args[2])..' Warrant'
        ...
        TriggerClientEvent('QBCore:Notify', src, 'You are not allowed to write warrants', 'error')
    end
end)
QBCore.Commands.Add('revokeWarrant', 'Revoke a warrant', { { name = 'CID', help = 'Target Player CitizenID' }}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == 'mayor' and Player.PlayerData.job.grade.level >= Config.LicenseRank then
		local SearchedPlayer = QBCore.Functions.GetPlayerByCitizenId(args[1])
		...
		local licenseTable = SearchedPlayer.PlayerData.metadata['licences']
		if not licenseTable['warrant'] then
			TriggerClientEvent('QBCore:Notify', src, 'Player does not have a warrant', 'error')
			return
		end
		licenseTable['warrant'] = false
		SearchedPlayer.Functions.SetMetaData('licences', licenseTable)
		TriggerClientEvent('QBCore:Notify', src, 'Warrant Revoked', 'success')
		exports['qb-inventory']:RemoveItem(src, 'warrant', 1, false, 'qb-police:revokeWarrant')
		TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['warrant'], 'remove')
        
    else
        TriggerClientEvent('QBCore:Notify', src, 'You are not allowed to handle warrants on this level', 'error')
    end
end)
...
QBCore.Commands.Add('resetillegalengine', 'POLICE ONLY - Reset the current vehicle mods', { name = 'price', help = Lang:t('info.impound_price') }, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local pedId = GetPlayerPed(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
		if GetVehiclePedIsIn(pedId, false) ~= 0 then
			TriggerClientEvent('police:client:getTheVehicle', src, args[1])
		else
			TriggerClientEvent('QBCore:Notify', src, 'You must do this in a vehicle', 'warning')
		end
    else
		TriggerClientEvent('QBCore:Notify', src, 'You are not a cop', 'warning')
		local v = src
		Wait(3000)
		TriggerEvent("qb-log:server:CreateLog", "anticheat", "Player Attempt to Reset Vehicle", "yellow", "**".. GetPlayerName(v) .. "** (<@"..(QBCore.Functions.GetIdentifier(v, 'discord'):gsub("discord:", "") or "unknown").."> |  ||"  ..(QBCore.Functions.GetIdentifier(v, 'ip') or 'undefined') ..  "|| | " ..(QBCore.Functions.GetIdentifier(v, 'license') or 'undefined') .." | " ..Player.PlayerData.citizenid.." | "..v..") kicked..")
		DropPlayer(src, 'Player Attempt to Reset Vehicle')
	end
end)
...
QBCore.Commands.Add('grantlicense', Lang:t('commands.license_grant'), { { name = 'id', help = Lang:t('info.player_id') }, { name = 'license', help = Lang:t('info.license_type') } }, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' or Player.PlayerData.job.name == 'mayor' and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add('revokelicense', Lang:t('commands.license_revoke'), { { name = 'id', help = Lang:t('info.player_id') }, { name = 'license', help = Lang:t('info.license_type') } }, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' or Player.PlayerData.job.type == 'mayor' and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == 'driver' or args[2] == 'weapon' then
            local SearchedPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
            if not SearchedPlayer then return end
            local licenseTable = SearchedPlayer.PlayerData.metadata['licences']
...
QBCore.Commands.Add('uncuffme', Lang:t('commands.cuff_player'), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
        TriggerClientEvent('police:client:unCuffPlayer', src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.on_duty_police_only'), 'error')
    end
end)
...
QBCore.Commands.Add('anklet', Lang:t('commands.anklet'), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == 'leo' or Player.PlayerData.job.type == 'mayor' and Player.PlayerData.job.onduty then
...
RegisterNetEvent('police:server:addVehicleItems', function(veh)
	local src = source
	local inventory = 'glovebox-'..tostring(veh)
	local data = {
		maxweight = 500000,
		slots = 10,
		label = inventory
	}
	exports['qb-inventory']:OpenInventory(src, inventory, data)
	Wait(100)
	exports['qb-inventory']:CloseInventory(src, inventory)
	local slotCount = 1
    for _, item in pairs(Config.CarItems) do
        local itemInfo = QBCore.Shared.Items[item]
        if itemInfo then
			if itemInfo.name == 'empty_evidence_bag' then
				exports['qb-inventory']:AddItem(inventory, itemInfo.name, 100, slotCount, itemInfo.info, 'PoliceJob - SetCarItemsInfo')
			elseif itemInfo.name == 'heavyarmor' then
				exports['qb-inventory']:AddItem(inventory, itemInfo.name, 3, slotCount, itemInfo.info, 'PoliceJob - SetCarItemsInfo')
			else
				exports['qb-inventory']:AddItem(inventory, itemInfo.name, 1, slotCount, itemInfo.info, 'PoliceJob - SetCarItemsInfo')
			end
			slotCount += 1
        end
    end
...
QBCore.Functions.CreateUseableItem('zipties', function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player.Functions.GetItemByName('zipties') then return end
    TriggerClientEvent('police:client:ZipPlayerSoft', src)
end)

QBCore.Functions.CreateUseableItem('handcuffs', function(source)
    -- local src = source
    -- local Player = QBCore.Functions.GetPlayer(src)
	-- if Player.PlayerData.job.name ~= 'police' and Player.PlayerData.job.onduty then TriggerClientEvent('QBCore:Notify', src, 'You lack the job role to use this item', 'error') return end
...
QBCore.Functions.CreateUseableItem('cuffpick', function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player.Functions.GetItemByName('cuffpick') then return end
    TriggerClientEvent('police:client:BreakCuffPlayer', src)
end)
...
RegisterNetEvent('police:server:startTheResetProcess', function(plate, price)
    local src = source
	local plate = plate
    local Player = QBCore.Functions.GetPlayer(src)
	local v = src
	local vehiclemods = '{"modAPlate":-1,"modLivery":-1,"modFrame":-1,"modWindows":-1,"neonColor":[255,0,255],"modDoorSpeaker":-1,"modTrunk":-1,"modBackWheels":-1,"modRearBumper":-1,"plateIndex":4,"headlightColor":[255,255,255],"modHydrolic":-1,"modPlateHolder":-1,"modRightFender":-1,"modKit21":-1,"modAerials":-1,"pearlescentColor":0,"modArmor":-1,"modFender":-1,"modSpoilers":-1,"modKit49":-1,"liveryRoof":-1,"modEngine":-1,"plate":"62CVN048","modDashboard":-1,"extras":[],"modAirFilter":-1,"modOrnaments":-1,"modTurbo":false,"modExhaust":-1,"modCustomTiresR":false,"modSteeringWheel":-1,"oilLevel":5,"modSpeakers":-1,"wheels":0,"modDial":-1,"windowTint":-1,"modKit47":-1,"modEngineBlock":-1,"modArchCover":-1,"modBProofTires":false,"dashboardColor":0,"modFrontWheels":-1,"modRoof":-1,"doorStatus":{"1":false,"2":false,"3":false,"4":false,"5":false,"0":false},"tireBurstCompletely":{"1":false,"2":false,"3":false,"4":false,"5":false,"0":false,"47":false,"45":false},"tyreSmokeColor":[255,255,255],"wheelSize":1.0,"modSeats":-1,"modTransmission":-1,"modFrontBumper":-1,"modXenon":false,"wheelWidth":1.0,"modGrille":-1,"modStruts":-1,"modSideSkirt":-1,"modHorns":-1,"windowStatus":{"1":false,"2":false,"3":false,"4":false,"5":false,"6":false,"7":false,"0":false},"engineHealth":987,"bodyHealth":720,"interiorColor":0,"tireHealth":{"1":1000.0,"2":1000.0,"3":1000.0,"4":0.0,"5":0.0,"0":999.0224609375,"47":0.0,"45":0.0},"modBrakes":-1,"modVanityPlate":-1,"color1":27,"modTrimA":-1,"modKit19":-1,"modTank":-1,"neonEnabled":[false,false,false,false],"tireBurstState":{"1":false,"2":false,"3":false,"4":false,"5":false,"0":false,"47":false,"45":false},"modShifterLeavers":-1,"modCustomTiresF":false,"model":1876913310,"modHood":-1,"wheelColor":0,"fuelLevel":81,"modSuspension":-1,"tankHealth":993,"color2":27,"modDrift":false,"modSmokeEnabled":false,"dirtLevel":6,"modKit17":-1,"modTrimB":-1}'
	MySQL.update('UPDATE player_vehicles SET mods = ? WHERE plate = ?', { vehiclemods, plate })
	MySQL.prepare('DELETE FROM `kq_tuning` WHERE `plate` = ? ', {plate}) 
	TriggerClientEvent('police:client:ImpoundVehicleReset', src, false, tonumber(price))
	TriggerClientEvent('QBCore:Notify', src, 'You reset the vehicle', 'success')
	TriggerEvent("qb-log:server:CreateLog", "vehicleupgrades", "Cop Reset Vehicle", "green", "**".. GetPlayerName(v) .. "** (<@"..(QBCore.Functions.GetIdentifier(v, 'discord'):gsub("discord:", "") or "unknown").."> |  ||"  ..(QBCore.Functions.GetIdentifier(v, 'ip') or 'undefined') ..  "|| | " ..(QBCore.Functions.GetIdentifier(v, 'license') or 'undefined') .." | " ..Player.PlayerData.citizenid.." | "..v..") Reset Plate:"..tostring(plate))
end)
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if #(playerCoords - targetCoords) > 5.0 then return DropPlayer(src, 'Attempted exploit abuse') end
...
RegisterNetEvent('police:server:unCuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local playerPed = GetPlayerPed(src)
    -- local targetPed = GetPlayerPed(playerId)
    -- local playerCoords = GetEntityCoords(playerPed)
    ...
    TriggerClientEvent('police:client:GetunCuffed', src, isSoftcuff)
end)

RegisterNetEvent('police:server:ZipPlayer', function(playerId, isSoftcuff)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local CuffedPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName('zipties') ) then return end
	
    if exports['qb-inventory']:RemoveItem(src, 'zipties', 1, false, 'used zips') then
        TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['zipties'], 'remove')
	end
    TriggerClientEvent('police:client:GetZipped', CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
end)

RegisterNetEvent('police:server:BreakCuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local CuffedPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not CuffedPlayer or not Player.Functions.GetItemByName('cuffpick') then return end

    if exports['qb-inventory']:RemoveItem(src, 'cuffpick', 1, false, 'Breaking Cuffs') then
        TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['cuffpick'], 'remove')
	end
    TriggerClientEvent('police:client:GetCuffed', CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
end)
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    local targetCoords = GetEntityCoords(targetPed)
    -- if #(playerCoords - targetCoords) > 2.5 then return DropPlayer(src, 'Attempted exploit abuse') end
	exports['sayer-gangs']:AddZoneRep(src, 'arresting')
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.type ~= 'leo' then return end
...
RegisterNetEvent('police:server:FlaggedPlateTriggered', function(camId, plate, street1, street2, blipSettings, playercoords)
    local src = source
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player then
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                ...
                if street2 ~= nil or street2 ~= '' then
                    local streetLabel = tostring(street1) .. " " .. tostring(street2) .. ' flagged plate reported: ' ..tostring(plate)
					
                    TriggerClientEvent("police:client:policeAlert", v, coords, streetLabel)
                    ...
                    local streetLabel = tostring(street1) .. ' flagged plate reported: ' ..tostring(plate)
                    TriggerClientEvent("police:client:policeAlert", v, coords, streetLabel)
                    -- TriggerClientEvent("112:client:SendPoliceAlert", v, "flagged", {
                        -- camId = camId,
                        -- plate = plate,
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
			local vehicleName = MySQL.scalar.await('SELECT vehicle FROM player_vehicles WHERE plate = ?', { plate })
			local vehType = QBCore.Shared.Vehicles[vehicleName] and QBCore.Shared.Vehicles[vehicleName].type or GetVehicleTypeByModel(vehicleName)
			if vehType == 'boat' then
				local garage = 'seadepot'
				MySQL.query(
					'UPDATE player_vehicles SET state = ?, depotprice = ?, body = ?, engine = ?, fuel = ?, garage = ? WHERE plate = ?',
					{ 0, price, body, engine, fuel, garage, plate })
				TriggerClientEvent('QBCore:Notify', src, Lang:t('info.vehicle_taken_depot', { price = price }))
			elseif vehType == 'heli' or vehType == 'plane' then
				local garage = 'airdepot'
				MySQL.query(
					'UPDATE player_vehicles SET state = ?, depotprice = ?, body = ?, engine = ?, fuel = ?, garage = ? WHERE plate = ?',
					{ 0, price, body, engine, fuel, garage, plate })
				TriggerClientEvent('QBCore:Notify', src, Lang:t('info.vehicle_taken_depot', { price = price }))
			else
				local garage = 'depotLot'
				MySQL.query(
					'UPDATE player_vehicles SET state = ?, depotprice = ?, body = ?, engine = ?, fuel = ?, garage = ? WHERE plate = ?',
					{ 0, price, body, engine, fuel, garage, plate })
				TriggerClientEvent('QBCore:Notify', src, Lang:t('info.vehicle_taken_depot', { price = price }))
...
RegisterNetEvent('evidence:server:CreateFingerDropInCar', function(plate, streetLabel)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	exports['qb-inventory']:OpenInventory(src, carInventory, data)
	Wait(100)
	exports['qb-inventory']:CloseInventory(src, carInventory)
	Wait(100)
    exports['qb-inventory']:AddItem(carInventory, 'filled_evidence_bag', 1, false, fingerInfo, 'evidence:server:CreateFingerDropInCar')
