...
RegisterNetEvent('police:client:ZipPlayerSoft', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QBCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                TriggerServerEvent('police:server:ZipPlayer', playerId, true)
                HandCuffAnimation()
            else
                QBCore.Functions.Notify(Lang:t('error.vehicle_cuff'), 'error')
            end
...
RegisterNetEvent('police:client:unCuffPlayer', function()
    if not IsPedRagdoll(PlayerPedId()) then
		...
		-- if not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) and not IsPedInAnyVehicle(PlayerPedId()) then
...
RegisterNetEvent('police:client:BreakCuffPlayer', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QBCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local result = QBCore.Functions.HasItem(Config.BreakCuffItem)
            if result then
                local playerId = GetPlayerServerId(player)
                if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                    TriggerServerEvent('police:server:BreakCuffPlayer', playerId, false)
                    HandCuffAnimation()
                else
                    QBCore.Functions.Notify(Lang:t('error.vehicle_cuff'), 'error')
                end
...
RegisterNetEvent('police:client:GetunCuffed', function(isSoftcuff)
    local ped = PlayerPedId()
    if not isHandcuffed then
        isHandcuffed = true
        TriggerServerEvent('police:server:SetHandcuffStatus', true)
...
RegisterNetEvent('police:client:GetZipped', function(playerId, isSoftcuff)
    local ped = PlayerPedId()
    if not isHandcuffed then
        isHandcuffed = true
        TriggerServerEvent('police:server:SetHandcuffStatus', true)
