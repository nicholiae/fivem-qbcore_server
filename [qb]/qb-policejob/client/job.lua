...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
			...
			TriggerServerEvent('police:server:addVehicleItems', QBCore.Functions.GetPlate(veh))
...
RegisterNetEvent('police:client:getTheVehicle', function( price )
	
    local vehicle = QBCore.Functions.GetClosestVehicle()
	local vehProps = QBCore.Functions.GetVehicleProperties(vehicle)
    TriggerServerEvent('police:server:startTheResetProcess', vehProps.plate, price)
	
end)
RegisterNetEvent('police:client:ImpoundVehicleReset', function(fullImpound, price)
    local vehicle = QBCore.Functions.GetClosestVehicle()
    local bodyDamage = math.ceil(GetVehicleBodyHealth(vehicle))
    local engineDamage = math.ceil(GetVehicleEngineHealth(vehicle))
    local totalFuel = exports[Config.FuelResource]:GetFuel(vehicle)
	local vehProps = QBCore.Functions.GetVehicleProperties(vehicle)
	vehProps.model = GetEntityModel(vehicle)
	vehProps.plate = GetVehicleNumberPlateText(vehicle)
	vehProps.plateIndex = GetVehicleNumberPlateTextIndex(vehicle)
	vehProps.bodyHealth = math.floor(GetVehicleBodyHealth(vehicle) + 0.5)
	vehProps.engineHealth = math.floor(GetVehicleEngineHealth(vehicle) + 0.5)
	vehProps.tankHealth = math.floor(GetVehiclePetrolTankHealth(vehicle) + 0.5)
	vehProps.fuelLevel = math.floor(GetVehicleFuelLevel(vehicle) + 0.5)
	vehProps.oilLevel = math.floor(GetVehicleOilLevel(vehicle) + 0.5)
	vehProps.dirtLevel = math.floor(GetVehicleDirtLevel(vehicle) + 0.5)
	-- vehProps.color1 = colorPrimary
	-- vehProps.color2 = colorSecondary
	-- vehProps.pearlescentColor = pearlescentColor
	vehProps.dashboardColor = GetVehicleDashboardColour(vehicle)
	-- vehProps.wheelColor = wheelColor
	vehProps.wheels = GetVehicleWheelType(vehicle)
	vehProps.wheelSize = GetVehicleWheelSize(vehicle)
	vehProps.wheelWidth = GetVehicleWheelWidth(vehicle)
	-- vehProps.tireHealth = -1
	-- vehProps.tireBurstState = -1
	-- vehProps.tireBurstCompletely = -1
	vehProps.windowTint = GetVehicleWindowTint(vehicle)
	-- vehProps.windowStatus = -1
	-- vehProps.doorStatus = -1
	-- vehProps.headlightColor = -1
	-- vehProps.neonEnabled = -1
	vehProps.neonColor = table.pack(GetVehicleNeonLightsColour(vehicle))
	vehProps.interiorColor = GetVehicleInteriorColour(vehicle)
	vehProps.tyreSmokeColor = table.pack(GetVehicleTyreSmokeColor(vehicle))
	vehProps.modSpoilers = -1
	vehProps.modFrontBumper = -1
	vehProps.modRearBumper = -1
	vehProps.modSideSkirt = -1
	vehProps.modExhaust = -1
	vehProps.modFrame = -1
	vehProps.modGrille = -1
	vehProps.modHood = -1
	vehProps.modFender = -1
	vehProps.modRightFender = -1
	vehProps.modRoof = -1
	vehProps.modEngine = -1
	vehProps.modBrakes = -1
	vehProps.modTransmission = -1
	vehProps.modHorns = -1
	vehProps.modSuspension = -1
	vehProps.modArmor = -1
	vehProps.modKit17 = -1
	vehProps.modTurbo = false
	vehProps.modKit19 = -1
	vehProps.modSmokeEnabled = false
	vehProps.modKit21 = -1
	vehProps.modXenon = false
	vehProps.modFrontWheels = -1
	vehProps.modBackWheels = -1
	vehProps.modCustomTiresF = GetVehicleModVariation(vehicle, 23)
	vehProps.modCustomTiresR = GetVehicleModVariation(vehicle, 24)
	vehProps.modPlateHolder = -1
	vehProps.modVanityPlate = -1
	vehProps.modTrimA = -1
	vehProps.modOrnaments = -1
	vehProps.modDashboard = -1
	vehProps.modDial = -1
	vehProps.modDoorSpeaker = -1
	vehProps.modSeats = -1
	vehProps.modSteeringWheel = -1
	vehProps.modShifterLeavers = -1
	vehProps.modAPlate = -1
	vehProps.modSpeakers = -1
	vehProps.modTrunk = -1
	vehProps.modHydrolic = -1
	vehProps.modEngineBlock = -1
	vehProps.modAirFilter = -1
	vehProps.modStruts = -1
	vehProps.modArchCover = -1
	vehProps.modAerials = -1
	vehProps.modTrimB = -1
	vehProps.modTank = -1
	vehProps.modWindows = -1
	vehProps.modKit47 = -1
	-- vehProps.modLivery = modLivery
	vehProps.modKit49 = -1
    vehProps.liveryRoof = GetVehicleRoofLivery(vehicle)
	vehProps.modDrift = false
	vehProps.modBProofTires = false
			
    if vehicle ~= 0 and vehicle then
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
        local vehpos = GetEntityCoords(vehicle)
        if #(pos - vehpos) < 5.0 and IsPedInAnyVehicle(ped) then
            QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                QBCore.Functions.SetVehicleProperties(vehicle, vehProps)
				TriggerServerEvent('qb-mechanicjob:server:SaveVehicleProps', vehProps)
...
RegisterNetEvent('qb-police:client:spawnHelicopterarmed', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
    else
        local coords = Config.Locations['helicopter2'][k]
        ...
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            TriggerEvent('vehiclekeys:client:SetOwner', QBCore.Functions.GetPlate(veh))
            SetVehicleEngineOn(veh, true, true)
        end, Config.PoliceHelicopterArmed, coords, true)
    end
end)
...
-- Helicopter Thread
local function heliarmed()
    CreateThread(function()
        while true do
            Wait(0)
            if inHelicopterArmed and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('qb-police:client:spawnHelicopterarmed')
                    break
                end
            else
                break
...
CreateThread(function()
    ...
    local helicopterZonesArmed = {}
    for i = 1, #Config.Locations['helicopter2'] do
        local v = Config.Locations['helicopter2'][i]
        helicopterZonesArmed[#helicopterZonesArmed + 1] = BoxZone:Create(
            vector3(v.x, v.y, v.z), 5, 5, {
                name = 'box_zone',
                debugPoly = false,
                minZ = v.z - 1,
    ...
    local helicopterComboArmed = ComboZone:Create(helicopterZonesArmed, { name = 'helicopterComboArmed', debugPoly = false })
    helicopterComboArmed:onPlayerInOut(function(isPointInside)
        if isPointInside then
			print("in this point")
            inHelicopterArmed = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['qb-core']:HideText()
                    exports['qb-core']:DrawText(Lang:t('info.store_heli'), 'left')
                                ...
                                if IsControlJustReleased(0, 38) then
                                    if IsPedInAnyVehicle(ped, false) then
                                        ...
                                        QBCore.Functions.SpawnVehicle(Config.Boat, function(veh)
                                            SetVehicleNumberPlateText(veh, "POBO"..tostring(math.random(1000, 9999)))
                                            SetEntityHeading(veh, coords.w)
