...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'qb-policejob' and invokingResource ~= 'qb-ambulancejob' and invokingResource ~= 'qb-policenpc' and invokingResource ~= GetCurrentResourceName() then
	...
	if not hasItem then
		QBCore.Functions.Notify('You have lost all illegal items from your inventory!!', 'warning')
		TriggerServerEvent('hospital:server:RemoveDeathItems')
	else
		TriggerServerEvent('hospital:server:RemoveBagItem')
		QBCore.Functions.Notify('Your freeBag has been consumed but your inventory maintained!!', 'warning')
	end
	Wait(5000)
