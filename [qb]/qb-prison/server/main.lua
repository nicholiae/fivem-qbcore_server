...
RegisterNetEvent('prison:server:SetJailStatus', function(jailTime)
    ...
    if jailTime > 0 then
		playerSavedJobs[citizenid] = Player.PlayerData.job
...
RegisterNetEvent('prison:server:GiveJailItems', function(escaped)
    ...
    Player.Functions.SetMetaData('jailitems', {})
	-- Restore job when released
	local citizenid = Player.PlayerData.citizenid
	if playerSavedJobs[citizenid] then
		Player.Functions.SetJob(playerSavedJobs[citizenid].name, playerSavedJobs[citizenid].grade)
		playerSavedJobs[citizenid] = nil
	end
    TriggerClientEvent('QBCore:Notify', src, 'You were given your job back')
end)
