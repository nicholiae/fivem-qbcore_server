	...
	if result ~= nil and result[1] ~= nil then
		for i = 1, #result do
			...
			if Amount > 0 then
				...
				TriggerEvent('qb-log:server:CreateLog', 'cryptocrash', 'Crypto E404', 'red', 
...
local function HandlePriceChance()
	...
	if Crypto.ChanceOfCrash == 404 then
		local lastChance = math.random(0,10)
		TriggerEvent('qb-log:server:CreateLog', 'cryptocrash', 'Crypto Error', 'orange', 
			-- 'Crypto Value: '..tostring(Crypto.Worth['qbit'])..
			'\nCrypto ChanceOfCrash: '..tostring(Crypto.ChanceOfCrash)..
			'\nCrypto lastChance: '..tostring(lastChance)
		)
		if lastChance <= 1 then
			resetAllCrypto()
		end
	end
	Crypto.ChanceOfCrash = math.random(0,808)
...
QBCore.Functions.CreateCallback('qb-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = QBCore.Functions.GetPlayer(source)
    local frst_price = math.floor((tonumber(data.Coins) * tonumber(Crypto.Worth['qbit'])))
    local tax_price = math.floor((tonumber(data.Coins) * tonumber(Crypto.Worth['qbit']))*tonumber(Crypto.PurchaseTax))
    local total_price = math.floor(frst_price + tax_price)
    if Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, Lang:t('credit.you_have_qbit_purchased', { dataCoins = tonumber(data.Coins) })..' for $'..tostring(total_price), 'Credit')
...
QBCore.Functions.CreateCallback('qb-crypto:server:SellCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, Lang:t('debit.you_have_sold', { dataCoins = tonumber(data.Coins) })..' for $'..tostring(amount), 'Debit')
...
CreateThread(function()
    while true do
        ...
        HandlePriceChance()
		TriggerEvent('qb-log:server:CreateLog', 'crypto', 'Crypto Update', 'white', 
			'Crypto Value: '..tostring(Crypto.Worth['qbit'])..
			'\nCrypto Upper: '..tostring(Crypto.Upper)..
			'\nCrypto Lower: '..tostring(Crypto.Lower)..
			'\nCrypto RefreshTimer: '..tostring(Crypto.RefreshTimer)..
			'\nCrypto ChanceOfCrash: '..tostring(Crypto.ChanceOfCrash)..
			'\nCrypto ChanceOfCrashOrLuck: '..tostring(Crypto.ChanceOfCrashOrLuck)..
			'\nCrypto Crash Max: '..tostring(Crypto.Crash[2])..
			'\nCrypto Crash Min: '..tostring(Crypto.Crash[1])..
			'\nCrypto Luck Max: '..tostring(Crypto.Luck[2])..
			'\nCrypto Luck Min: '..tostring(Crypto.Luck[1])..
			'\nCrypto ChanceOfDown: '..tostring(Crypto.ChanceOfDown)..
			'\nCrypto ChanceOfUp: '..tostring(Crypto.ChanceOfUp)..
			'\nCrypto CasualDown Max: '..tostring(Crypto.CasualDown[2])..
			'\nCrypto CasualDown Min: '..tostring(Crypto.CasualDown[1])..
			'\nCrypto CasualUp Max: '..tostring(Crypto.CasualUp[2])..
			'\nCrypto CasualUp Min: '..tostring(Crypto.CasualUp[1])
		)
    end
end)
