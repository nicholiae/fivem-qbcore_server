...
RegisterNetEvent('qb-trashsearch:server:openTrash', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
	
	exports['qb-inventory']:ClearStash('publictrash')
	
	Wait(1000) -- wait  to allow deletion
	
    exports['qb-inventory']:OpenInventory(src, 'publictrash', {
        maxweight = 4000000,
        slots = 300,
    })
end)
