...
local function ejectFromVehicle()
	...
	if random1 == random2 then
        ...
        QBCore.Functions.Notify("You were saved by the grace of God!", 'primary')
	end
end
                ...
                if lastFrameVehSpeed > 110 and thisFrameVehSpeed < (lastFrameVehSpeed * 0.75) and not damageDone then
                    if frameBodyChange > 18.0 then
                        ...
                        elseif (seatbeltOn or harnessOn) and not IsThisModelABike(currVehicle) then
                            if lastFrameVehSpeed > 180 then
                                if math.random(math.ceil(lastFrameVehSpeed)) > 180 then
                                    if not harnessOn then
                                        ejectFromVehicle()
                                    else
                                        harnessHp -= 1
...
RegisterCommand('toggleseatbelt', function()
    ...
    if class == 13 or class == 14 then return end
    toggleSeatbelt()
end, false)
