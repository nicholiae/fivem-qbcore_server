...
CreateThread(function()
    while true do
        ...
        if IsPedBeingStunned(ped, 0) then
            sleep = 0
            SetPedMinGroundTimeForStungun(ped, math.random(5000, 10000))
        end
        Wait(sleep)
    end
end)
...
CreateThread(function()
	while true do
		...
		local lvl = 5
		QBCore.Functions.TriggerCallback('qb-smallresources:server:getCops', function(cops)
			if cops >= Config.PoliceCount then
				lvl = 0
				-- print(lvl)
				SetMaxWantedLevel(lvl)
			else	
				...
				Wait(150)	
				if playerJob == 'police' or playerJob == 'mayor' then
					lvl = 0
				else
					lvl = 5
				end
