...
local function SaveSkin()
    ...
    -- Trigger sync to nearby players
    -- TriggerServerEvent("qb-clothing:server:syncOutfitToNearby", model, clothing)
end
...
RegisterNetEvent('qb-clothing:client:outsideOpenMenu', function(allowedMenus)
    customCamLocation = nil
    openMenu(allowedMenus)
end)
...
-- Sync clothing from other players
RegisterNetEvent('qb-clothing:client:syncPlayerClothing', function(targetSource, model, skinDataJson)
    -- This event is triggered when another player's clothing needs to be synced
    local targetPlayer = GetPlayerFromServerId(targetSource)
    
    if targetPlayer == -1 or targetPlayer == PlayerId() then
        return -- Player not in range or is self
    end
    
    local targetPed = GetPlayerPed(targetPlayer)
    
    if targetPed == 0 or not DoesEntityExist(targetPed) then
        return -- Ped doesn't exist yet
    end
    
    -- Decode the skin data
    local skinData = type(skinDataJson) == "string" and json.decode(skinDataJson) or skinDataJson
    
    if not skinData then
        return
    end
    
    -- Apply the clothing to the target player's ped
    Citizen.CreateThread(function()
        -- Wait a bit to ensure the ped is fully loaded
        Wait(100)
        
        -- Apply all clothing components
        TriggerEvent('qb-clothing:client:loadPlayerClothing', skinData, targetPed)
    end)
end)
...
RegisterNetEvent('qb-clothing:client:loadOutfit', function(oData)
	...
	-- print(oData)
	local oData = oData
	-- print("Data after: "..tostring(oData))
    ...
    if data["accessory"] ~= nil then
        if QBCore.Functions.GetPlayerData().metadata["tracker"] then
            SetPedComponentVariation(ped, 7, 78, 0, 0)
        else
            SetPedComponentVariation(ped, 7, data["accessory"].item, data["accessory"].texture, 0)
        end
    else
	...
	-- Sync outfit change to nearby players
    local model = GetEntityModel(PlayerPedId())
    local clothing = json.encode(skinData)
    -- TriggerServerEvent("qb-clothing:server:syncOutfitToNearby", model, clothing)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('qb-clothes:chargeForOutfit', ped)
                ...
                if inZone then
                    ...
                    if zoneName == 'surgeon' then
                        if IsControlJustReleased(0, 38) then
                            customCamLocation = nil
                            TriggerServerEvent('qb-clothing:server:chargeforsurgeon')
                            openMenu({
                                {menu = "character", label = Lang:t("menu.features"), selected = true},
                            })
                        end
...
-- Proximity-based clothing refresh (safety net)
-- Citizen.CreateThread(function()
    -- while true do
        ...
        -- for _, player in ipairs(players) do
            -- if player ~= PlayerId() then
                ...
                -- if DoesEntityExist(targetPed) then
                    ...
                    -- Request clothing refresh for nearby players
                    -- if distance < 50.0 then
                        -- local targetServerId = GetPlayerServerId(player)
                        -- if targetServerId > 0 then
                            -- TriggerServerEvent('qb-clothing:server:requestPlayerClothing', targetServerId)
                        -- end
                    -- end
                -- end
            -- end
