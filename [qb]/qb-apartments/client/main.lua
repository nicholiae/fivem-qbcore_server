...
local IsInsideToiletZone = false
local IsInsideShowerZone = false
local IsInsideSinkZone = false
...
local function RegisterInApartmentZone(targetKey, coords, heading, text)
    ...
    zone:onPlayerInOut(function(isPointInside)
        ...
        if targetKey == 'sinkPos' then
            IsInsideSinkZone = isPointInside
        end
		
        if targetKey == 'toiletPos' then
            IsInsideToiletZone = isPointInside
        end

        if targetKey == 'showerPos' then
            IsInsideShowerZone = isPointInside
        end
    end)
...
local function SetInApartmentTargets()
	...
	local showerPos = vector3(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.shower.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.shower.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.shower.z)
    local toiletPos = vector3(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.toilet.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.toilet.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.toilet.z)
    local sinkPos = vector3(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.sink.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.sink.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.sink.z)
	
	ApartmentDataSend = {['showerPos'] = showerPos, ['toiletPos'] = toiletPos, ['sinkPos'] = sinkPos}
    ...
    if UseTarget then
        ...
        RegisterInApartmentTarget('sinkPos', sinkPos, 0, {
            {
                type = 'client',
                event = 'PlayerBiology:client:useApartmentSink',
                label = "Sink",
            },
        })
        RegisterInApartmentTarget('toiletPos', toiletPos, 0, {
            {
                type = 'client',
                event = 'PlayerBiology:client:useApartmentToilet',
                label = "Toilet",
            },
        })
        RegisterInApartmentTarget('showerPos', showerPos, 0, {
            {
                type = 'client',
                event = 'PlayerBiology:client:useApartmentShower',
                label = "Shower",
            },
        })
    else
        ...
        RegisterInApartmentZone('toiletPos', toiletPos, 0, '[E] To use Toilet')
        RegisterInApartmentZone('showerPos', showerPos, 0, '[E] To use Shower')
        RegisterInApartmentZone('sinkPos', sinkPos, 0, '[E] To use Sink')
...
local function DeleteInApartmentTargets()
    ...
    IsInsideSinkZone = false
    IsInsideShowerZone = false
    IsInsideToiletZone = false
    ...
    CreateThread(function()
        ...
        while true do
            ...
            elseif InApartment then
                ...
                if IsInsideShowerZone then
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent('PlayerBiology:client:useApartmentShower', ApartmentDataSend)
                        exports['qb-core']:HideText()
                    end
                end
				
                if IsInsideSinkZone then
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent('PlayerBiology:client:useApartmentSink', ApartmentDataSend)
                        exports['qb-core']:HideText()
                    end
                end
				
                if IsInsideToiletZone then
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent('PlayerBiology:client:useApartmentToilet', ApartmentDataSend)
                        exports['qb-core']:HideText()
                    end
                end
            end
...
RegisterCommand('getapartmentoffset', function()
    local coords = GetEntityCoords(PlayerPedId())
    local head = GetEntityHeading(PlayerPedId())
	print(ClosestHouse)
	print(Apartments.Locations[ClosestHouse].coords.enter.x)
	print(Apartments.Locations[ClosestHouse].coords.enter.y)
	print(Apartments.Locations[ClosestHouse].coords.enter.z)
	
    local houseCoords = vector3(
        Apartments.Locations[ClosestHouse].coords.enter.x,
        Apartments.Locations[ClosestHouse].coords.enter.y,
        Apartments.Locations[ClosestHouse].coords.enter.z
    )
    if houseCoords then
        ...
        print('Y: ' .. ydist)
        print('Z: ' .. zdist)
		print(Apartments.Locations[ClosestHouse].coords.enter.x,Apartments.Locations[ClosestHouse].coords.enter.y,Apartments.Locations[ClosestHouse].coords.enter.z)
		local data = tostring('{"x": '..tostring(xdist)..', "y": '..tostring(ydist)..', "z": '..tostring(zdist)..', "h":'..tostring(head)..'}')
		print(data)
		CopyToClipboard('{"x": '..tostring(xdist)..', "y": '..tostring(ydist)..', "z": '..tostring(zdist)..', "h":'..tostring(head)..'}')
    end
end)
