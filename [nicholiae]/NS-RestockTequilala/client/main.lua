...
-- zone check
local jobcheck_Tequilala = false
local entranceTargetID = 'entranceTarget_Tequilala'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitTarget_Tequilala'
local isInsideExitZone = false
local exitZone = nil

local deliveryTargetID = 'deliveryTarget_Tequilala'
local isInsideDeliveryZone = false
local deliveryZone = nil

local dutyTargetID = 'dutyTarget_Tequilala'
local isInsideDutyZone = false
local dutyZone = nil

local pickupTargetID = 'pickupTarget_Tequilala'
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	...
	CreateThread(function()
		for key, value in pairs(Config_Tequilala.approvedJobs) do
			if value == QBCore.Functions.GetPlayerData().job.name then
				local blip = AddBlipForCoord(Config_Tequilala.OutsideLocation_Tequilala.x, Config_Tequilala.OutsideLocation_Tequilala.y, Config_Tequilala.OutsideLocation_Tequilala.z)
				...
				AddTextComponentSubstringPlayerName(value..' Imports')
...
local function DestroyPickupTarget()
    ...
    if Config_Tequilala.UseTarget then
...
local function RegisterEntranceTarget()
	for key, value in pairs(Config_Tequilala.approvedJobs) do
		...
		if value == QBCore.Functions.GetPlayerData().job.name then
			jobcheck_Tequilala = true
		end
	end
	if jobcheck_Tequilala == true then
		local coords = vector3(Config_Tequilala.OutsideLocation_Tequilala.x, Config_Tequilala.OutsideLocation_Tequilala.y, Config_Tequilala.OutsideLocation_Tequilala.z)
	
		if Config_Tequilala.UseTarget then
			entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
				name = entranceTargetID,
				heading = 44.0,
				minZ = Config_Tequilala.OutsideLocation_Tequilala.z - 1.0,
				maxZ = Config_Tequilala.OutsideLocation_Tequilala.z + 2.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockTequilala:client:target:enterLocation',
...
local function RegisterExitTarget()
	...
	if jobcheck_Tequilala == true then
		local coords = vector3(Config_Tequilala.InsideLocation_Tequilala.x, Config_Tequilala.InsideLocation_Tequilala.y, Config_Tequilala.InsideLocation_Tequilala.z)

		if Config_Tequilala.UseTarget then
			exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
				name = exitTargetID,
				heading = 270,
				minZ = Config_Tequilala.InsideLocation_Tequilala.z - 1.0,
				maxZ = Config_Tequilala.InsideLocation_Tequilala.z + 2.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockTequilala:client:target:exitLocation',
...
local function RegisterDutyTarget()
	...
	if jobcheck_Tequilala == true then
		local coords = vector3(Config_Tequilala.DutyLocation.x, Config_Tequilala.DutyLocation.y, Config_Tequilala.DutyLocation.z)

		if Config_Tequilala.UseTarget then
			dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
				name = dutyTargetID,
				heading = 270,
				minZ = Config_Tequilala.DutyLocation.z - 2.0,
				maxZ = Config_Tequilala.DutyLocation.z + 1.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockTequilala:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
	...
	if jobcheck_Tequilala == true then
		local coords = vector3(Config_Tequilala.DropLocation.x, Config_Tequilala.DropLocation.y, Config_Tequilala.DropLocation.z)

		if Config_Tequilala.UseTarget then
			deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
				name = deliveryTargetID,
				heading = 270,
				minZ = Config_Tequilala.DropLocation.z - 2.0,
				maxZ = Config_Tequilala.DropLocation.z + 1.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockTequilala:client:target:dropPackage',
...
local function GetRandomPackage()
    packageCoords = Config_Tequilala.PickupLocations[math.random(1, #Config_Tequilala.PickupLocations)]
...
local function PickupPackage()
    ...
    RequestModel(Config_Tequilala.PickupBoxModel)
    while not HasModelLoaded(Config_Tequilala.PickupBoxModel) do
        Wait(0)
    end
    local object = CreateObject(Config_Tequilala.PickupBoxModel, pos.x, pos.y, pos.z, true, true, true)
...
-- local function SetLocationBlip()
    -- local RecycleBlip = AddBlipForCoord(Config_Tequilala.OutsideLocation_Tequilala.x, Config_Tequilala.OutsideLocation_Tequilala.y, Config_Tequilala.OutsideLocation_Tequilala.z)
    -- SetBlipSprite(RecycleBlip, 365)
    -- SetBlipColour(RecycleBlip, 2)
    -- SetBlipScale(RecycleBlip, 0.8)
    -- SetBlipAsShortRange(RecycleBlip, true)
    -- BeginTextCommandSetBlipName('STRING')
    -- AddTextComponentSubstringPlayerName('Customs Center')
    -- EndTextCommandSetBlipName(RecycleBlip)
-- end

local function buildInteriorDesign()
	if jobcheck_Tequilala == true then
		for _, pickuploc in pairs(Config_Tequilala.PickupLocations) do
			local model = GetHashKey(Config_Tequilala.WarehouseObjects[math.random(1, #Config_Tequilala.WarehouseObjects)])
			RequestModel(model)
...
local function EnterLocation()
	...
	for key, value in pairs(Config_Tequilala.approvedJobs) do
		...
		if value == QBCore.Functions.GetPlayerData().job.name then
			...
			SetEntityCoords(PlayerPedId(), Config_Tequilala.InsideLocation_Tequilala.x, Config_Tequilala.InsideLocation_Tequilala.y, Config_Tequilala.InsideLocation_Tequilala.z)
			buildInteriorDesign()
			DoScreenFadeIn(500)
	...
	-- QBCore.Functions.Notify('The RT is not for you', 'error')
end
...
local function ExitLocation()
	...
	if jobcheck_Tequilala == true then
		...
		SetEntityCoords(PlayerPedId(), Config_Tequilala.OutsideLocation_Tequilala.x, Config_Tequilala.OutsideLocation_Tequilala.y, Config_Tequilala.OutsideLocation_Tequilala.z + 1)
		DoScreenFadeIn(500)
		...
		if Config_Tequilala.UseTarget then
			pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
				...
				options = {
					{
						type = 'client',
						event = 'NS-RestockTequilala:client:target:pickupPackage',
...
local function DrawPackageLocationBlip()
	if jobcheck_Tequilala == true then
		if not Config_Tequilala.DrawPackageLocationBlip then
			return
		end
...
RegisterNetEvent('NS-RestockTequilala:client:target:enterLocation', function()
    EnterLocation()
end)

RegisterNetEvent('NS-RestockTequilala:client:target:exitLocation', function()
    ExitLocation()
end)

RegisterNetEvent('NS-RestockTequilala:client:target:toggleDuty', function()
...
RegisterNetEvent('NS-RestockTequilala:client:target:pickupPackage', function()
    if not pickupZone or carryPackage then
        return
    end

    if not Config_Tequilala.UseTarget and not isInsidePickupZone then
        return
    end

    QBCore.Functions.Progressbar('pickup_reycle_package', Lang:t('text.picking_up_the_package'), Config_Tequilala.PickupActionDuration, false, true, {
...
RegisterNetEvent('NS-RestockTequilala:client:target:dropPackage', function()
    if not carryPackage or not deliveryZone then
        return
    end

    if not Config_Tequilala.UseTarget and not isInsideDeliveryZone then
        return
    end
    ...
    ScrapAnim()
    DestroyDeliveryTarget()
    QBCore.Functions.Progressbar('deliver_reycle_package', Lang:t('text.unpacking_the_package'), Config_Tequilala.DeliveryActionDuration, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true
        ...
        -- Done
        StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
        TriggerServerEvent('NS-RestockTequilala:server:getItem')
        GetRandomPackage()
    end)
end)
        ...
        while true do
            ...
            if isInsideEntranceZone then
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    Wait(500)
                    TriggerEvent('NS-RestockTequilala:client:target:enterLocation')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if isInsideExitZone then
                ...
                if IsControlJustReleased(0, 38) then
                    exports['qb-core']:KeyPressed()
                    Wait(500)
                    TriggerEvent('NS-RestockTequilala:client:target:exitLocation')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if isInsideDutyZone then
                ...
                if IsControlJustReleased(0, 38) then
                    exports['qb-core']:KeyPressed()
                    Wait(500)
                    TriggerEvent('NS-RestockTequilala:client:target:toggleDuty')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if onDuty then
                if isInsidePickupZone and not carryPackage then
                    ...
                    if IsControlJustReleased(0, 38) then
                        exports['qb-core']:KeyPressed()
                        Wait(500)
                        TriggerEvent('NS-RestockTequilala:client:target:pickupPackage')
                        exports['qb-core']:HideText()
                    end
                ...
                if isInsideDeliveryZone and carryPackage then
                    ...
                    if IsControlJustReleased(0, 38) then
                        exports['qb-core']:KeyPressed()
                        Wait(500)
                        TriggerEvent('NS-RestockTequilala:client:target:dropPackage')
                        exports['qb-core']:HideText()
                    end
                end
            end
