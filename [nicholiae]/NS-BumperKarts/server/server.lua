...
local index = 1
local players = 1
-- local modelChoice = math.random(1, )
local vehicleList = {}
for model, vehicleData in pairs(QBCore.Shared.Vehicles) do
	if vehicleData.type == 'automobile' then
		vehicleList[#vehicleList+1] = vehicleData.hash
	end
end
local modelChoice = vehicleList[math.random(1, #vehicleList)]
...
function startGame()
		-- modelChoice = vehicleList[math.random(1, #vehicleList)]
		-- cfg.bumperKarts.vehicleModelList = modelChoice
		modelChoice = cfg.bumperKarts.vehicleModelList[math.random(1, #cfg.bumperKarts.vehicleModelList)]
        for k, v in pairs(playerQueue) do 
            if tablelength(existingKarts) == tablelength(playerQueue) then
                ...
                local bumperKart = Citizen.InvokeNative(`CREATE_AUTOMOBILE` & 0xFFFFFFFF, modelChoice, cfg.bumperKarts.locations[k], 360.0)
                Citizen.Wait(200)
...
RegisterNetEvent('NS-BumperKarts:server:payForPlay', function()
    local src = source
	...
	if playerCash < cfg.costToPlay then
		...
		elseif playerBank >= cfg.costToPlay then
			TriggerClientEvent('QBCore:Notify', src, "Thank you for your payment, bumperkarts will begin soon!", 'success')
			Player.Functions.RemoveMoney('bank', cfg.costToPlay, 'NS-BumperKarts:server:payForPlay')
		end
	elseif playerCash >= cfg.costToPlay then
		TriggerClientEvent('QBCore:Notify', src, "Thank you for your payment, bumperkarts will begin soon!", 'success')
		Player.Functions.RemoveMoney('cash', cfg.costToPlay, 'NS-BumperKarts:server:payForPlay')
	end
end)
