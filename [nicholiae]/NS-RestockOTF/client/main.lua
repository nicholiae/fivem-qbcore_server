...
local onDuty = false
local jobCheck_OTF = false
-- zone check

local entranceTargetID = 'entranceTarget_OTF'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitTarget_OTF'
local isInsideExitZone = false
local exitZone = nil

local deliveryTargetID = 'deliveryTarget_OTF'
local isInsideDeliveryZone = false
local deliveryZone = nil

local dutyTargetID = 'dutyTarget_OTF'
local isInsideDutyZone = false
local dutyZone = nil

local pickupTargetID = 'pickupTarget_OTF'
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	...
	CreateThread(function()
		for key, value in pairs(Config_OTF.approvedJobs) do
			if value == QBCore.Functions.GetPlayerData().gang.name then
				local blip = AddBlipForCoord(Config_OTF.OutsideLocation_OTF.x, Config_OTF.OutsideLocation_OTF.y, Config_OTF.OutsideLocation_OTF.z)
...
local function RegisterEntranceTarget()
	...
	if jobCheck_OTF == true then
		local coords = vector3(Config_OTF.OutsideLocation_OTF.x, Config_OTF.OutsideLocation_OTF.y, Config_OTF.OutsideLocation_OTF.z)
	
		if Config_OTF.UseTarget then
			entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
				name = entranceTargetID,
				heading = 44.0,
				minZ = Config_OTF.OutsideLocation_OTF.z - 1.0,
				maxZ = Config_OTF.OutsideLocation_OTF.z + 2.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockOTF:client:target:enterLocation',
...
local function RegisterExitTarget()
	if jobCheck_OTF == true then
		local coords = vector3(Config_OTF.InsideLocation_OTF.x, Config_OTF.InsideLocation_OTF.y, Config_OTF.InsideLocation_OTF.z)

		if Config_OTF.UseTarget then
			exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
				name = exitTargetID,
				heading = 270,
				minZ = Config_OTF.InsideLocation_OTF.z - 1.0,
				maxZ = Config_OTF.InsideLocation_OTF.z + 2.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockOTF:client:target:exitLocation',
...
local function RegisterDutyTarget()
    local coords = vector3(Config_OTF.DutyLocation.x, Config_OTF.DutyLocation.y, Config_OTF.DutyLocation.z)

    if Config_OTF.UseTarget then
        dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
            name = dutyTargetID,
            heading = 270,
            minZ = Config_OTF.DutyLocation.z - 2.0,
            maxZ = Config_OTF.DutyLocation.z + 1.0,
            debugPoly = false,
        }, {
            options = {
                {
                    type = 'client',
                    event = 'NS-RestockOTF:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
    local coords = vector3(Config_OTF.DropLocation.x, Config_OTF.DropLocation.y, Config_OTF.DropLocation.z)

    if Config_OTF.UseTarget then
        deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
            name = deliveryTargetID,
            heading = 270,
            minZ = Config_OTF.DropLocation.z - 2.0,
            maxZ = Config_OTF.DropLocation.z + 1.0,
            debugPoly = false,
        }, {
            options = {
                {
                    type = 'client',
                    event = 'NS-RestockOTF:client:target:dropPackage',
...
local function GetRandomPackage()
    packageCoords = Config_OTF.PickupLocations[math.random(1, #Config_OTF.PickupLocations)]
...
local function PickupPackage()
    ...
    RequestModel(Config_OTF.PickupBoxModel)
    while not HasModelLoaded(Config_OTF.PickupBoxModel) do
        Wait(0)
    end
    local object = CreateObject(Config_OTF.PickupBoxModel, pos.x, pos.y, pos.z, true, true, true)
...
-- local function SetLocationBlip()
    -- local RecycleBlip = AddBlipForCoord(Config_OTF.OutsideLocation_OTF.x, Config_OTF.OutsideLocation_OTF.y, Config_OTF.OutsideLocation_OTF.z)
...
local function buildInteriorDesign()
    for _, pickuploc in pairs(Config_OTF.PickupLocations) do
        local model = GetHashKey(Config_OTF.WarehouseObjects[math.random(1, #Config_OTF.WarehouseObjects)])
        RequestModel(model)
...
local function EnterLocation()
	...
	for key, value in pairs(Config_OTF.approvedJobs) do
		...
		if value == QBCore.Functions.GetPlayerData().gang.name then
			...
			SetEntityCoords(PlayerPedId(), Config_OTF.InsideLocation_OTF.x, Config_OTF.InsideLocation_OTF.y, Config_OTF.InsideLocation_OTF.z)
			buildInteriorDesign()
			DoScreenFadeIn(500)
...
local function ExitLocation()
    ...
    SetEntityCoords(PlayerPedId(), Config_OTF.OutsideLocation_OTF.x, Config_OTF.OutsideLocation_OTF.y, Config_OTF.OutsideLocation_OTF.z + 1)
    DoScreenFadeIn(500)
    ...
    if Config_OTF.UseTarget then
        pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
            ...
            options = {
                {
                    type = 'client',
                    event = 'NS-RestockOTF:client:target:pickupPackage',
...
local function DrawPackageLocationBlip()
    if not Config_OTF.DrawPackageLocationBlip then
        return
    end
...
RegisterNetEvent('NS-RestockOTF:client:target:enterLocation', function()
    EnterLocation()
end)

RegisterNetEvent('NS-RestockOTF:client:target:exitLocation', function()
    ExitLocation()
end)

RegisterNetEvent('NS-RestockOTF:client:target:toggleDuty', function()
    onDuty = not onDuty
    if onDuty then
...
RegisterNetEvent('NS-RestockOTF:client:target:pickupPackage', function()
    if not pickupZone or carryPackage then
        return
    end

    if not Config_OTF.UseTarget and not isInsidePickupZone then
        return
    end

    QBCore.Functions.Progressbar('pickup_reycle_package', Lang:t('text.picking_up_the_package'), Config_OTF.PickupActionDuration, false, true, {
...
RegisterNetEvent('NS-RestockOTF:client:target:dropPackage', function()
    if not carryPackage or not deliveryZone then
        return
    end

    if not Config_OTF.UseTarget and not isInsideDeliveryZone then
        return
    end
    ...
    ScrapAnim()
    DestroyDeliveryTarget()
    QBCore.Functions.Progressbar('deliver_reycle_package', Lang:t('text.unpacking_the_package'), Config_OTF.DeliveryActionDuration, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true
        ...
        -- Done
        StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
        TriggerServerEvent('NS-RestockOTF:server:getItem')
        GetRandomPackage()
    end)
end)
        ...
        while true do
            ...
            if isInsideEntranceZone then
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    Wait(500)
                    TriggerEvent('NS-RestockOTF:client:target:enterLocation')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if isInsideExitZone then
                ...
                if IsControlJustReleased(0, 38) then
                    exports['qb-core']:KeyPressed()
                    Wait(500)
                    TriggerEvent('NS-RestockOTF:client:target:exitLocation')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if isInsideDutyZone then
                ...
                if IsControlJustReleased(0, 38) then
                    exports['qb-core']:KeyPressed()
                    Wait(500)
                    TriggerEvent('NS-RestockOTF:client:target:toggleDuty')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if onDuty then
                if isInsidePickupZone and not carryPackage then
                    ...
                    if IsControlJustReleased(0, 38) then
                        exports['qb-core']:KeyPressed()
                        Wait(500)
                        TriggerEvent('NS-RestockOTF:client:target:pickupPackage')
                        exports['qb-core']:HideText()
                    end
                ...
                if isInsideDeliveryZone and carryPackage then
                    ...
                    if IsControlJustReleased(0, 38) then
                        exports['qb-core']:KeyPressed()
                        Wait(500)
                        TriggerEvent('NS-RestockOTF:client:target:dropPackage')
                        exports['qb-core']:HideText()
                    end
                end
            end
