...
window.addEventListener('message', function(event) {
  // vérifier si le type de message est "toggleMenuRADIO"
  if (event.data.type === 'toggleMenuRADIO') {
    toggleMenuRADIO();
  }
});
...
function closeMenuRADIO() {
  $.post(`https://${GetParentResourceName()}/menuClosedRADIO`, JSON.stringify({}), function() {
  });
}

function ONRADIO() {
  $.post(`https://${GetParentResourceName()}/StartRADIO`, JSON.stringify({}), function() {
    // Callback function (si nécessaire)
  });
}
function OFFRADIO() {
  $.post(`https://${GetParentResourceName()}/StopRADIO`, JSON.stringify({}), function() {
    // Callback function (si nécessaire)
  });
}