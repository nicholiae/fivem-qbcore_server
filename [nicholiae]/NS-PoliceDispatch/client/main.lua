...
-- CreateThread(function()
	...
	-- AddTextComponentSubstringPlayerName('Police Dispatcher')
	...
	while true do
		...
		if dist <= 25.0 then
			...
			if not DoesEntityExist(evidence) then
				...
				evidence = CreatePed(26, Config.DispatchPed, Config.EvidenceCoords.x, Config.EvidenceCoords.y, Config.EvidenceCoords.z, Config.EvidenceCoords.w, false, false)
...
CreateThread(function()
	...
	while playerDeathCheck == true do
		...
		if IsPedDeadOrDying(player) == 1 then
			...
			TriggerEvent('NS-PoliceDispatch:CleanUp')
...
function MissionNotification(VehicleCoords)
	...
	TriggerServerEvent('qb-phone:server:sendNewMail', {
		sender = "Dispatch",
		subject = "Dispatch to All Units",
		message = "All units please respond, disturbance at "..tostring(street1name .. ' ' .. street2name),
	})
end
---
--
RegisterNetEvent('NS-PoliceDispatch:SpawnMissionAssets', function()
	...
	CreateThread(function()
		...
		while not spawned do
			...
			if distance < 200 then
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentSubstringPlayerName('Disturbance')
...
local function GetCuffedAnimation(playerId, pedID)
    local ped = pedID
...
function startMission()
	CreateThread(function()
		while MissionStart == 1 do
			...
			local plyCoords = GetEntityCoords(PlayerPedId(), false)
			local transCoords = GetEntityCoords(navigator)
			...
			if dist <= 25.0  then
				...
				if warning == 0 then
					warning = 1
					stopAndBeAngry()
					QBCore.Functions.Notify("Stop the Criminal!", "error")
				end
				...
				if dist <= 7 and GuardsDead == 1 then
					if BlowBackdoor == 0 then
						hintToDisplay('Press [G] to arrest the Agressor')
	...
	if IsVehicleStopped(transport) then
		if GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				ClearPedTasks(pilot)
				TaskWanderStandard(pilot, 100,100)
				Wait(5500)
				ClearPedTasks(PlayerPedId())
				hideLastHint()
				QBCore.Functions.Notify('Arrested Successfully', "error")
				Wait(600)
				FreezeEntityPosition(PlayerPedId(), false)
				BlownUp = 1
				lootable = 1
				QBCore.Functions.Notify('You can start loading the criminal.', "success")
...
CreateThread(function()
    while true do
		...
		if lootable == 1 then
			...
			if dist <= 2.5 then
				sleep = 0
				hintToDisplay('Press [E] to Stow the Criminal')
...
RegisterNetEvent('NS-PoliceDispatch:CleanUp', function()
	DeleteEntity(pilot)
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify('You are stowing the criminal', "success")
	...
	while GetGameTimer() - _time < 20000 do
		...
		hintToDisplay('Hold [G] to let them Die')
		Wait(0)
	end
	...
	TriggerServerEvent("NS-PoliceDispatch:GiveLoot", LootTime)
	TriggerEvent('NS-PoliceDispatch:CleanUp')
	Wait(2500)
end
