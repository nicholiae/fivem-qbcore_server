...
local function IsVehicleOwned(plate)
	...
	print('IsVehicleOwned:'..tostring(plate))
...
QBCore.Functions.CreateCallback('NS-Insurance:server:getHouseGarage', function(_, cb, house)
    local houseInfo = MySQL.single.await('SELECT * FROM houselocations WHERE name = ?', { house })
    cb(houseInfo)
end)

QBCore.Functions.CreateCallback('NS-Insurance:server:GetGarageVehicles', function(source, cb, garage, type, category)
...
QBCore.Functions.CreateCallback('NS-Insurance:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
...
QBCore.Functions.CreateCallback('NS-Insurance:server:canPay', function(source, cb, price)
    local Player = QBCore.Functions.GetPlayer(source)
    local charge = price
...
QBCore.Functions.CreateCallback('NS-Insurance:server:canDeposit', function(source, cb, plate, type, garage, state)
    local Player = QBCore.Functions.GetPlayer(source)
    local isOwned = MySQL.scalar.await('SELECT citizenid FROM player_vehicles WHERE plate = ?', { plate })
	
	
	if Config.Debug then	
		print('canDeposit:'..tostring(plate))
		print('canDeposit:'..tostring(source))
		print('canDeposit:'..tostring(isOwned))
		print('canDeposit:'..tostring(Player.PlayerData.citizenid))
	end
...
RegisterNetEvent('NS-Insurance:server:updateVehicleStats', function(plate, fuel, engine, body)
    local src = source
...
RegisterNetEvent('NS-Insurance:server:updateVehicleState', function(state, plate)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('NS-Insurance:server:UpdateOutsideVehicle', function(plate, vehicleNetID)
...
RegisterNetEvent('NS-Insurance:server:PayClaim', function(price)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local bankBalance = Player.PlayerData.money['bank']
	if bankBalance >= price then
		Player.Functions.RemoveMoney('bank', price, 'paid-Claim')
	else
...
RegisterNetEvent('NS-Insurance:server:syncGarage', function(updatedGarages)
    Config.Garages = updatedGarages
end)

--Call from qb-phone

QBCore.Functions.CreateCallback('NS-Insurance:server:GetPlayerVehicles', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    local Vehicles = {}
