...
RegisterCommand("help", function(source, args, raw)
	if (QBCore.Functions.GetPlayerData().metadata["isdead"]) or (QBCore.Functions.GetPlayerData().metadata["inlaststand"]) and spam then
		QBCore.Functions.TriggerCallback('hhfw:docOnline', function(EMSOnline, hasEnoughMoney)
			if EMSOnline <= Config.Doctor and hasEnoughMoney and not spam then
...
RegisterCommand("resetHelp", function(source, args, raw)
	if spam then
		QBCore.Functions.TriggerCallback('hhfw:docOnline', function(EMSOnline)
			Active = false
			test = nil
			test1 = nil
			spam = true
			Notify("Successfully reset your doctor", "error")
		end)
	else
		Notify("Please Wait for the Doctor to reset", "error")
	end
end)
...
function SpawnVehicle(x, y, z)  
	...
	-- if not DoesEntityExist(vehhash) then
        ...
        mechPed = CreatePed(26, GetHashKey('s_m_m_doctor_01'), loc.x+ math.random(-spawnRadius, spawnRadius),loc.y+ math.random(-spawnRadius, spawnRadius),loc.z,0, true, false)              	
		...
		Wait(2000)
		TaskGoToCoordAnyMeans(mechPed, loc.x, loc.y, loc.z, 21.0, 0, 0, 786603, 0xbf800000)
		-- TaskGoToCoordAnyMeans(mechPed, mechVeh, loc.x, loc.y, loc.z, 20.0, 0, GetEntityModel(mechVeh), 524863, 2.0)
		-- test = mechVeh
		test1 = mechPed
		Active = true
    -- end
