...
local onDuty = false
local jobCheck_Businesses = false
-- zone check

local entranceTargetID = 'entranceTarget_Business'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitTarget_Business'
local isInsideExitZone = false
local exitZone = nil

local deliveryTargetID = 'deliveryTarget_Business'
local isInsideDeliveryZone = false
local deliveryZone = nil

local dutyTargetID = 'dutyTarget_Business'
local isInsideDutyZone = false
local dutyZone = nil

local pickupTargetID = 'pickupTarget_Business'
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	...
	CreateThread(function()
		for key, value in pairs(Config_Business.approvedJobs) do
			if value == QBCore.Functions.GetPlayerData().job.name then
				local blip = AddBlipForCoord(Config_Business.OutsideLocation_Business.x, Config_Business.OutsideLocation_Business.y, Config_Business.OutsideLocation_Business.z)
...
local function DestroyPickupTarget()
    ...
    if Config_Business.UseTarget then
...
local function RegisterEntranceTarget()
	...
	for key, value in pairs(Config_Business.approvedJobs) do
		...
		if value == QBCore.Functions.GetPlayerData().job.name then
			jobCheck_Businesses = true
		end
	end
	if jobCheck_Businesses == true then
		local coords = vector3(Config_Business.OutsideLocation_Business.x, Config_Business.OutsideLocation_Business.y, Config_Business.OutsideLocation_Business.z)
	
		if Config_Business.UseTarget then
			entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
				name = entranceTargetID,
				heading = 44.0,
				minZ = Config_Business.OutsideLocation_Business.z - 1.0,
				maxZ = Config_Business.OutsideLocation_Business.z + 2.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockBusiness:client:target:enterLocation',
...
local function RegisterExitTarget()
	if jobCheck_Businesses == true then
		local coords = vector3(Config_Business.InsideLocation_Business.x, Config_Business.InsideLocation_Business.y, Config_Business.InsideLocation_Business.z)

		if Config_Business.UseTarget then
			exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
				name = exitTargetID,
				heading = 270,
				minZ = Config_Business.InsideLocation_Business.z - 1.0,
				maxZ = Config_Business.InsideLocation_Business.z + 2.0,
				debugPoly = false,
			}, {
				options = {
					{
						type = 'client',
						event = 'NS-RestockBusiness:client:target:exitLocation',
...
local function RegisterDutyTarget()
    local coords = vector3(Config_Business.DutyLocation.x, Config_Business.DutyLocation.y, Config_Business.DutyLocation.z)

    if Config_Business.UseTarget then
        dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
            name = dutyTargetID,
            heading = 270,
            minZ = Config_Business.DutyLocation.z - 2.0,
            maxZ = Config_Business.DutyLocation.z + 1.0,
            debugPoly = false,
        }, {
            options = {
                {
                    type = 'client',
                    event = 'NS-RestockBusiness:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
    local coords = vector3(Config_Business.DropLocation.x, Config_Business.DropLocation.y, Config_Business.DropLocation.z)

    if Config_Business.UseTarget then
        deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
            name = deliveryTargetID,
            heading = 270,
            minZ = Config_Business.DropLocation.z - 2.0,
            maxZ = Config_Business.DropLocation.z + 1.0,
            debugPoly = false,
        }, {
            options = {
                {
                    type = 'client',
                    event = 'NS-RestockBusiness:client:target:dropPackage',
...
local function GetRandomPackage()
    packageCoords = Config_Business.PickupLocations[math.random(1, #Config_Business.PickupLocations)]
...
local function PickupPackage()
    ...
    RequestModel(Config_Business.PickupBoxModel)
    while not HasModelLoaded(Config_Business.PickupBoxModel) do
        Wait(0)
    end
    local object = CreateObject(Config_Business.PickupBoxModel, pos.x, pos.y, pos.z, true, true, true)
...
-- local function SetLocationBlip()
    -- local RecycleBlip = AddBlipForCoord(Config_Business.OutsideLocation_Business.x, Config_Business.OutsideLocation_Business.y, Config_Business.OutsideLocation_Business.z)
    -- SetBlipSprite(RecycleBlip, 365)
    -- SetBlipColour(RecycleBlip, 2)
    -- SetBlipScale(RecycleBlip, 0.8)
    -- SetBlipAsShortRange(RecycleBlip, true)
    -- BeginTextCommandSetBlipName('STRING')
    -- AddTextComponentSubstringPlayerName('Retail Center')
    -- EndTextCommandSetBlipName(RecycleBlip)
-- end

local function buildInteriorDesign()
    for _, pickuploc in pairs(Config_Business.PickupLocations) do
        local model = GetHashKey(Config_Business.WarehouseObjects[math.random(1, #Config_Business.WarehouseObjects)])
        RequestModel(model)
...
local function EnterLocation()
	...
	for key, value in pairs(Config_Business.approvedJobs) do
		...
		if value == QBCore.Functions.GetPlayerData().job.name then
			...
			SetEntityCoords(PlayerPedId(), Config_Business.InsideLocation_Business.x, Config_Business.InsideLocation_Business.y, Config_Business.InsideLocation_Business.z)
			buildInteriorDesign()
			DoScreenFadeIn(500)
	...
	-- QBCore.Functions.Notify('The RB is not for you', 'error')
end
...
local function ExitLocation()
    ...
    SetEntityCoords(PlayerPedId(), Config_Business.OutsideLocation_Business.x, Config_Business.OutsideLocation_Business.y, Config_Business.OutsideLocation_Business.z + 1)
    DoScreenFadeIn(500)
    ...
    if Config_Business.UseTarget then
        pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
            ...
            options = {
                {
                    type = 'client',
                    event = 'NS-RestockBusiness:client:target:pickupPackage',
...
local function DrawPackageLocationBlip()
    if not Config_Business.DrawPackageLocationBlip then
        return
    end
...
RegisterNetEvent('NS-RestockBusiness:client:target:enterLocation', function()
    EnterLocation()
end)

RegisterNetEvent('NS-RestockBusiness:client:target:exitLocation', function()
    ExitLocation()
end)

RegisterNetEvent('NS-RestockBusiness:client:target:toggleDuty', function()
    onDuty = not onDuty
    if onDuty then
...
RegisterNetEvent('NS-RestockBusiness:client:target:pickupPackage', function()
    if not pickupZone or carryPackage then
        return
    end

    if not Config_Business.UseTarget and not isInsidePickupZone then
        return
    end

    QBCore.Functions.Progressbar('pickup_reycle_package', Lang:t('text.picking_up_the_package'), Config_Business.PickupActionDuration, false, true, {
...
RegisterNetEvent('NS-RestockBusiness:client:target:dropPackage', function()
    if not carryPackage or not deliveryZone then
        return
    end

    if not Config_Business.UseTarget and not isInsideDeliveryZone then
        return
    end
    ...
    ScrapAnim()
    DestroyDeliveryTarget()
    QBCore.Functions.Progressbar('deliver_reycle_package', Lang:t('text.unpacking_the_package'), Config_Business.DeliveryActionDuration, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true
        ...
        -- Done
        StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
        TriggerServerEvent('NS-RestockBusiness:server:getItem')
        GetRandomPackage()
    end)
end)
        ...
        while true do
            ...
            if isInsideEntranceZone then
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    Wait(500)
                    TriggerEvent('NS-RestockBusiness:client:target:enterLocation')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if isInsideExitZone then
                ...
                if IsControlJustReleased(0, 38) then
                    exports['qb-core']:KeyPressed()
                    Wait(500)
                    TriggerEvent('NS-RestockBusiness:client:target:exitLocation')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if isInsideDutyZone then
                ...
                if IsControlJustReleased(0, 38) then
                    exports['qb-core']:KeyPressed()
                    Wait(500)
                    TriggerEvent('NS-RestockBusiness:client:target:toggleDuty')
                    exports['qb-core']:HideText()
                end
            end
            ...
            if onDuty then
                if isInsidePickupZone and not carryPackage then
                    ...
                    if IsControlJustReleased(0, 38) then
                        exports['qb-core']:KeyPressed()
                        Wait(500)
                        TriggerEvent('NS-RestockBusiness:client:target:pickupPackage')
                        exports['qb-core']:HideText()
                    end
                ...
                if isInsideDeliveryZone and carryPackage then
                    ...
                    if IsControlJustReleased(0, 38) then
                        exports['qb-core']:KeyPressed()
                        Wait(500)
                        TriggerEvent('NS-RestockBusiness:client:target:dropPackage')
                        exports['qb-core']:HideText()
                    end
                end
            end
