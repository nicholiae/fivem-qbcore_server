...
function ChargeForEngine(player, engineKey, workshop)
    ...
    if workshop.useSharedAccount then
        local bossSystem = Config.SharedBankingSystem or 'qb-banking'
    
        if bossSystem == 'qb-banking' then
            if exports[bossSystem]:GetAccountBalance(workshop.sharedAccount) >= engine.price then
                exports[bossSystem]:RemoveMoney(workshop.sharedAccount, engine.price, 'Engine purchase')
                return true
            else
            ...
            if exports[bossSystem]:GetAccount(workshop.sharedAccount) >= engine.price then
                if bossSystem == 'qb-bossmenu' then
