...
-- CreateThread(function()
    -- local FIBElevator = AddBlipForCoord(BlipMarkerLoc.x, BlipMarkerLoc.y, BlipMarkerLoc.z)
    -- SetBlipSprite(FIBElevator, 304)
    -- SetBlipDisplay(FIBElevator, 4)
    -- SetBlipScale(FIBElevator, 0.7)
    -- SetBlipAsShortRange(FIBElevator, true)
    -- SetBlipColour(FIBElevator, 2)
    -- BeginTextCommandSetBlipName('STRING')
    -- AddTextComponentSubstringPlayerName('FIB Heist')
    -- EndTextCommandSetBlipName(FIBElevator)
-- end)
...
function SetBoobyTrap(stat)
	local boobyStatus = stat
	TriggerServerEvent("AlenFIB:boobyTrap", boobyStatus)
end

local function CreateFire(Firecoords, elapsed)
	for _ = 1, math.random(1, 7), 1 do
		TriggerServerEvent("FIB:StartServerFire", Firecoords, 24, false)
		TriggerEvent("FIB:StartFire", Firecoords, 24, false)
	end
	Wait(elapsed)
	trapSet = false
	SetBoobyTrap(trapSet)
	TriggerServerEvent("FIB:StopFires")
end

RegisterNetEvent('FIB:StartFire')
AddEventHandler('FIB:StartFire', function(coords, maxChildren, isGasFire)
	-- print(coords)
...
RegisterNetEvent('FIB:StopFires')
AddEventHandler('FIB:StopFires', function()
...
function SpawnAirportVehicle(outCoords)
	
	coords = {x = outCoords.x, y = outCoords.y, z = outCoords.z, h = outCoords.w}
	QBCore.Functions.SpawnVehicle(Config['SanVeh'], function(veh)
		...
		SetEntityHeading(veh, outCoords.w)
		exports['myFuel']:SetFuel(veh, 100.0)
		TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(veh))
		SetVehicleEngineOn(veh, true, true)
		plaquevehicule = GetVehicleNumberPlateText(veh)
...
CreateThread(function()
	...
	Citizen.CreateThread(function()
		while true do
			...
			local policeupcoords = #(fetchmyped - vector3(124.94, -725.03, 47.08))
			local policedowncoords = #(fetchmyped - vector3(147.2, -737.94, 242.15))
			...
			if inMission == true then
				if IsPedDeadOrDying(ped) == 1 or (QBCore.Functions.GetPlayerData().metadata["isdead"]) or (QBCore.Functions.GetPlayerData().metadata["inlaststand"]) then
				   ...
				   DrawText3D(136.16516, -762.0228, 45.752021, "~b~[E]~w~To Go Upstairs")
					if IsControlJustPressed(0, 38) then
						-- if Cops >= Config['RequiredCops'] then		
							SpawnPeds()
							QBCore.Functions.TriggerCallback('AlenFIB:checkFirst', function(hasit)
								-- print(hasit)
								if hasit then
									...
									function() 
										for k,v in pairs(Config.BlackListJobs) do
											if v == Job then
												...
												-- QBCore.Functions.Notify('You probably already know why you cant do this heist...', 'error')
												QBCore.Functions.Notify('E404', 'error')
												doNotPort = true
												
											end
										end
										if doNotPort == true then
											TriggerServerEvent('FIB:SendAReport')
										else
									...
									QBCore.Functions.Notify('This mission is already underway, come back later', 'error')
								end
							end)			
						-- else
			...
			-- if policeupcoords <= 5.0 and Job == Config['Police'] then
			if policedowncoords <= 5.0 then
				if Job == Config['Police'] then
					sleep = 1
					DrawMarker(26, 124.94, -725.03, 47.08, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 1.0, 237, 197, 66, 255, false, false)
					if policedowncoords <= 2.0 then
					   DrawText3D(147.2, -737.94, 242.15, "~b~[E]~w~To Go DownStairs")
						if IsControlJustPressed(0, 38) then
							TeleportPolicePlayerOut()
							inMission = false
						end
					end
				end
			end
			if policeupcoords <= 5.0 then
				QBCore.Functions.TriggerCallback('AlenFIB:BoobyCheck', function(hasit)
					trapSet = hasit
					-- print("IN CALLBACK "..tostring(trapSet))
				end)
				-- print("OUT CALLBACK "..tostring(trapSet))
				if Job == Config['Police'] then
					sleep = 1
					DrawMarker(26, 124.94, -725.03, 47.08, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 1.0, 237, 197, 66, 255, false, false)
					if policeupcoords <= 2.0 then
					   DrawText3D(124.94, -725.03, 47.08, "~b~[E]~w~To Go Upstairs")
						if IsControlJustPressed(0, 38) then
							TriggerServerEvent('FIB:MakeAReportOfPoliceEnteance')
							if trapSet then
								QBCore.Functions.Notify('Its a Trap', 'error')
								local Firecoords = GetEntityCoords(PlayerPedId())
								local elapse = Config.FireTime * 1000
								CreateFire(Firecoords,elapse)
							else
								TeleportPolicePlayer()
								inMission = true
							end
						end
					end
					...
					if not trapSet then
						sleep = 1
						DrawMarker(26, 124.94, -725.03, 47.08, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 1.0, 237, 197, 66, 255, false, false)
						if policeupcoords <= 2.0 and QBCore.Functions.HasItem('thermite') then
						   DrawText3D(124.94, -725.03, 47.08, "~b~[E]~w~To Set A Trap")
							if IsControlJustPressed(0, 38) then
								SetBoobyTrap(true)
								trapSet = true
								QBCore.Functions.Notify('Succesfully set a Trap', 'success')
								TriggerServerEvent('AlenFIB:RemoveThermite')
								TriggerEvent('qb-inventory:client:ItemBox', ped, QBCore.Shared.Items['thermite'], 'remove')
							end
						end
					end
				end
									...
									if dist <= 1.5 then
										...
										if IsControlJustReleased(0, 51) then
											if Cops >= Config['RequiredCops'] then
												...
												local level = 5
												local message = "You are now Most-Wanted for Robbing the FIB"
												-- QBCore.Functions.Notify(message, 'error')
												TriggerEvent('NS-Wantedhandler:client:addStars', pid, level, message)
												...
												if QBCore.Functions.HasItem(Config.breakThroughItems[1]) and QBCore.Functions.HasItem(Config.breakThroughItems[2]) and QBCore.Functions.HasItem(Config.breakThroughItems[3]) then
													TriggerServerEvent('AlenFIB:OpenDoor')
													PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
													QBCore.Functions.Notify('Succesfully  Unlocked The FIB Vault!', 'success')
													TriggerServerEvent('AlenFIB:RemoveBreakThroughItems')
												else 
													TriggerServerEvent('AlenFIB:PoliceCall')
									...
									if dist <= 1.0 then
										...
										if IsControlJustReleased(0, 51) then
											...
											if doNotPort == true then
												...
												SetEntityCoords(PlayerPedId(), 1970.15, 211.06, 2699.59)
											else
												TriggerServerEvent('AlenFIB:PoliceCall')
												GrabCash(v['Obj'], k)
											end
...
function SpawnPeds()
	...
	if not DoesRelationshipGroupExist(GetHashKey('GuardPeds')) then
    ...
    for k, v in pairs(Config['AttackingPeds']['PedConfig']) do
		...
		if DoesRelationshipGroupExist(GetHashKey('GuardPeds')) then
			...
			if v['ped'] ~= 'a_f_m_prolhost_01' then
				-- print('guard')
				SetPedAccuracy(FIBHeist['heistPeds'][k], 65)
				SetPedArmour(FIBHeist['heistPeds'][k], 200)
				...
				if random == 2 then
					GiveWeaponToPed(FIBHeist['heistPeds'][k], GetHashKey('weapon_assaultsmg'), 255, false, false)
					TaskGuardCurrentPosition(FIBHeist['heistPeds'][k], 10.0, 10.0, 1)
				else
					GiveWeaponToPed(FIBHeist['heistPeds'][k], GetHashKey('weapon_combatshotgun'), 255, false, false)
					TaskGuardCurrentPosition(FIBHeist['heistPeds'][k], 10.0, 10.0, 1)
				end
			else
				-- print('hands up')
...
function TeleportPolicePlayer()
    DoScreenFadeOut(2000)
    while not IsScreenFadedOut() do
        Citizen.Wait(50)
    end
    local ped = PlayerPedId()
    SetEntityCoords(ped, 147.18, -738.01, 242.15)
    SetEntityHeading(ped, 251.66)
	
   
	SetRelationshipBetweenGroups(0, GetHashKey("GuardPeds"), GetHashKey("PLAYER"))
	SetRelationshipBetweenGroups(0, GetHashKey("PLAYER"), GetHashKey("GuardPeds"))
	
    DoScreenFadeIn(2000)
end

function TeleportPolicePlayerOut()
    DoScreenFadeOut(2000)
    while not IsScreenFadedOut() do
        Citizen.Wait(50)
    end
    local ped = PlayerPedId()
    SetEntityCoords(ped, 124.94, -725.03, 47.08)
    SetEntityHeading(ped, 251.66)
...
function TeleportPlayerSanAndreas()
	...
	inMission = false
    local ped = PlayerPedId()
	local randomlocation = math.random(1,4)
	if randomlocation == 1 then
		outCoords = vector4(-308.15, -757.67, 38.78, 155.06)
		SetEntityCoords(ped, -306.8749, -743.1676, 38.780178)
		SetEntityHeading(ped, 161.97296)
	elseif randomlocation == 2 then
		outCoords = vector4(47.92, -694.99, 31.58, 157.4)
		SetEntityCoords(ped, 37.76, -694.86, 31.71)
		SetEntityHeading(ped, 253.04)
	elseif randomlocation == 3 then
		outCoords = vector4(194.31, -599.73, 29.41, 243.96)
		SetEntityCoords(ped, 190.73, -607.21, 29.58)
		SetEntityHeading(ped, 330.73)
	elseif randomlocation == 4 then
		outCoords = vector4(215.83, -938.71, 24.14, 142.48)
		SetEntityCoords(ped, 211.56, -934.83, 24.28)
		SetEntityHeading(ped, 221.86)
	end
    SpawnAirportVehicle(outCoords)
    DoScreenFadeIn(2000)	
end

CreateThread(function()
	local coords = vector3(124.86, -725.03, 47.08)
	local trapZone = BoxZone:Create(coords, 1.0, 1.0, {
		name = 'PoliceEntrance',
		heading = 165.82,
		minZ = 47.08 - 1,
		maxZ = 47.08 + 1,
	...
	trapZone:onPlayerInOut(function(inside)
		-- print(inside)
		if inside then
		   local requiredItems = {
			...
			TriggerEvent('qb-inventory:client:requiredItems', requiredItems, show)
		else
			local requiredItems = {
				[1] = { name = QBCore.Shared.Items['thermite']['name'], image = QBCore.Shared.Items['thermite']['image'] },
			}
...
CreateThread(function()
	local coords = vec3(128.13, -729.37, 241.17)
	local vaultZone = BoxZone:Create(coords, 1.0, 1.0, {
		name = 'Vault Entrance',
		heading = 69.99938694844,
		minZ = 239.08 - 1,
		maxZ = 243.08 + 1,
		debugPoly = false
	})
	vaultZone:onPlayerInOut(function(inside)
		-- print(inside)
		if inside then
		   local requiredItems = {
