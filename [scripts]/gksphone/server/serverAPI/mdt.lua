...
Config.Core.Functions.CreateCallback('gksphone:server:ScanPlate', function(source, cb, plate)
	...
	local vehicleData = {}
	if plate ~= nil then
		local result = MySQL.Sync.fetchAll('SELECT * FROM '..Config.OwnedVehicles..' WHERE plate = ?', { plate })
		if result[1] ~= nil then
...
Config.Core.Functions.CreateCallback('gksphone:server:GetVehicleSearchResults', function(source, cb, search)
	...
	local searchData = {}
	local query = '%' .. search .. '%'
	local result = MySQL.Sync.fetchAll('SELECT * FROM '..Config.OwnedVehicles..' WHERE plate LIKE ? OR citizenid = ?',
		{ query, search })
	if result[1] ~= nil then
		for k, v in pairs(result) do
