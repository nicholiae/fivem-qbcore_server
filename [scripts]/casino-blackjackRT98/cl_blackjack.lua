...
RegisterNetEvent("doj:client:startingBets", function(args)
    ...
    if waitingForBetState then
        if args == 1 then 
            ...
            currentBetAmount = currentBetAmount + 10000
        elseif args == 2 then 
            -- -- print("Bet lowered")
            TriggerEvent("doj:client:openBetMenu")
            if currentBetAmount >= 10000 then 
                currentBetAmount = currentBetAmount - 10000
            else
        ...
        elseif args == 5 then 
            -- -- print("Bet raised")
            TriggerEvent("doj:client:openBetMenu")
            currentBetAmount = currentBetAmount + 100000
        else
            -- print('exit')
    ...
    while true do
        if drawTimerBar then
            QBCore.Functions.TriggerCallback('BLACKJACKRT98:server:blackChipsAmount', function(result)
                retval = result
				if timeLeft < 10 then
					exports['casinoUi']:DrawCasinoUi('show', "<b>The Diamond Casino & Resort Blackjack</b></p>Time Left: 0:0"..timeLeft.."</p>Availble chips: "..math.floor(result).."</p>Current Bet: "..math.floor(currentBetAmount))   
				else
