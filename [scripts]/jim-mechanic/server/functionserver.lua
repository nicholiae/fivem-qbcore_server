...
---==[[ CHECK CASH FOR REPAIRS ]]==---
	RegisterNetEvent('jim-mechanic:chargeCash', function(cost, society)
		...
		local amount = math.floor(cost + 0.5)
		...
		elseif currentBank >= cost then
			Core.Functions.GetPlayer(source).Functions.RemoveMoney("bank", cost)
		end
		...
		if Config.System.RenewedBanking then
			...
			exports["qb-banking"]:AddMoney(society, math.ceil(cost - (cost / 4)))
		end
	end)
