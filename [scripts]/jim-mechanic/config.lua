Config = {
	...
	Main = { -- Main variables
		...
		JobRoles = { "mechanic","customs","bikes","pjsauto","mechj","tuners","tow" }, -- These are the job roles who can use items if RequiresJob is enabled.
	...
	Overrides = {
		...
		updateServerDelay = 5, 	-- default 20 second database update delay from the LAST upgrade done to a vehicle
	...
	vehFailure = { 						-- Enabling these will make allow you to no longer need qb-vehiclefailure
		...
		damageLimits = {
			petrolTank = 500.0,			-- Prevents tankHealth ever going below ( if this goes too low fuel leaks and instantly destorys the engine)
	...
	Odometer = {
		...
		odoCustomisation = {
			showHealthValues = true,								-- Always show health numbers on icons (otherwise only shows in debug mode)
	...
	NOS = { -- NOS STUF
		...
		EnableFlame = false, 		-- True adds exhaut flame effects while boosting
	...
	Discord = { -- Discord preview receipts
		...
		DiscordDefault = "https://discord.com/api/webhooks/1331818162827563090/Ie8T_ljnhUAt0iYEZD5CjxRNn7yKoqTdkij_MZpfOgsZVkOZ5jcnI6A-fi7uUxGiYTan", 		-- Set this to the default channel API link if one isn't set for a location
