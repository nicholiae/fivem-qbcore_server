...
AddEventHandler('qb-zombies:moneyloot', function()
	...
	TriggerClientEvent('qb-weapons:client:SetWeaponQuality', source, 100)
...
AddEventHandler('qb-zombies:itemloot', function(listKey)
    ...
    for k,v in pairs(Config.Items) do
        if item == k then
			...
			TriggerClientEvent('qb-weapons:client:SetWeaponQuality', src, 100)
			TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[v], 'add')
			TriggerClientEvent("QBCore:Notify", source, 'You found ' .. Config.ItemAmount .. ' ' .. v .. '(s)','success')
        end
    end
